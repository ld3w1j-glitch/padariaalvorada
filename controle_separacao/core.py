...
PRINT_TEMPLATE_DIR = os.path.join(BASE_DIR, "templates_impressao")
PRINT_TEMPLATE_PATH = os.path.join(PRINT_TEMPLATE_DIR, "Base.xlsx")
...
SENSITIVE_ACCESS_KEYS = {"usuarios", "configuracoes", "codigo_fonte", "auditoria"}
NO_REPORT_ACCESS_ROLES = {"separador", "conferente"}
FULL_ACCESS_ROLES = {"gerente", "desenvolvedor", "supervisor"}
ROLE_LABELS = {
    "admin": "Admin",
    "gerente": "Gerente",
    "estoque": "Estoque",
...
DEFAULT_ACCESS_BY_ROLE: dict[str, set[str]] = {
    ...
    "gerente": set(ACCESS_KEYS) - SENSITIVE_ACCESS_KEYS,
    "estoque": {"painel", "estoque", "recebimentos", "balanco", "relatorios", "lotes", "impressao_rapida", "mcp_teste", "comunicacao"},
    "separador": {"painel", "separacoes", "pedidos", "lotes", "impressao_rapida", "comunicacao"},
    "conferente": {"painel", "recebimentos", "separacoes", "pedidos", "lotes", "impressao_rapida", "comunicacao"},
    "balanco": {"painel", "estoque", "balanco", "relatorios", "mcp_teste", "comunicacao"},
    "desenvolvedor": set(ACCESS_KEYS) - SENSITIVE_ACCESS_KEYS,
    "supervisor": set(ACCESS_KEYS) - SENSITIVE_ACCESS_KEYS,
...
def normalize_role(value: Any) -> str:
    ...
    allowed = {"admin", "gerente", "estoque", "separador", "conferente", "balanco", "desenvolvedor", "supervisor", "visualizador"}
...
def sanitize_access_rules_for_role(accesses: Iterable[str], role: Any, *, keep_admin_sensitive: bool = False) -> set[str]:
    """Normaliza permissões manuais respeitando bloqueios obrigatórios por cargo."""
    role_norm = normalize_role(role)
    sanitized = {str(item).strip().lower() for item in accesses if str(item).strip().lower() in ACCESS_KEYS}
    if role_norm != "admin" or not keep_admin_sensitive:
        sanitized -= SENSITIVE_ACCESS_KEYS
    if role_norm in NO_REPORT_ACCESS_ROLES:
        sanitized.discard("relatorios")
    return sanitized
...
def default_access_rules(role: Any, permission_level: Any = "comum") -> set[str]:
    ...
    return sanitize_access_rules_for_role(DEFAULT_ACCESS_BY_ROLE.get(role_norm, {"painel"}), role_norm)
...
def parse_access_rules(raw: Any, role: Any = None, permission_level: Any = "comum") -> set[str]:
    if isinstance(raw, (list, tuple, set)):
        ...
        try:
            ...
            return default_access_rules(role, permission_level)
    acessos = sanitize_access_rules_for_role(valores, role, keep_admin_sensitive=(normalize_role(role) == "admin"))
    if not acessos:
        return default_access_rules(role, permission_level)
    return acessos
...
def user_is_admin(user: sqlite3.Row | dict[str, Any] | None) -> bool:
    ...
    Somente o cargo Admin libera informações sensíveis como usuários,
    configurações, código-fonte e auditoria. O nível de permissão não
    transforma gerente/supervisor/desenvolvedor em admin sensível.
    """
    if user is None:
        return False
...
def user_access_set(user: sqlite3.Row | dict[str, Any] | None) -> set[str]:
    ...
    # Informações sensíveis são somente do cargo Admin real.
    acessos -= SENSITIVE_ACCESS_KEYS
    # Separador e conferente nunca acessam Relatórios, mesmo que a permissão
    # tenha ficado marcada em um cadastro antigo.
    if normalize_role(role) in NO_REPORT_ACCESS_ROLES:
        acessos.discard("relatorios")
    # Chat/Tarefas e Área do usuário são áreas comuns: todo usuário logado pode acessar.
    acessos.add("comunicacao")
    acessos.add("area_usuario")
    return acessos
...
def user_has_access(user: sqlite3.Row | dict[str, Any] | None, module: str) -> bool:
    ...
    if module_key not in ACCESS_KEYS:
        return False
    if module_key in {"comunicacao", "area_usuario"} and user is not None:
        return True
    if user is not None and module_key == "relatorios" and normalize_role(user["role"] if "role" in user.keys() else None) in NO_REPORT_ACCESS_ROLES:
        return False
    if module_key in SENSITIVE_ACCESS_KEYS and not user_is_admin(user):
        return False
...
def can_edit_stock_registration(user: sqlite3.Row | dict[str, Any] | None) -> bool:
    if user is None:
        return False
    return user_has_access(user, "estoque") and (user_is_admin(user) or normalize_role(user["role"] if "role" in user.keys() else None) in FULL_ACCESS_ROLES)
...
CREATE TABLE IF NOT EXISTS user_area_tasks (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER NOT NULL,
    titulo TEXT NOT NULL,
    meta TEXT,
...
CREATE TABLE IF NOT EXISTS user_area_notes (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER NOT NULL,
    titulo TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS erp_stock_imports (
    ...
    conn.execute("CREATE INDEX IF NOT EXISTS idx_user_area_tasks_user_status_prazo ON user_area_tasks(user_id, status, prazo)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_user_area_notes_user ON user_area_notes(user_id)")
    ...
    for user in users:
        ...
        permission_level = normalize_permission_level(user["permission_level"], role_norm)
        access_rules = parse_access_rules(user["access_rules"], role_norm, permission_level)
        if role_norm == "admin":
            permission_level = "admin"
            access_rules = set(ACCESS_KEYS)
        else:
            permission_level = "comum"
            access_rules -= SENSITIVE_ACCESS_KEYS
            if role_norm in {"separador", "conferente", "estoque", "gerente"}:
                if role_norm in {"conferente", "estoque"}:
                    access_rules.add("recebimentos")
                access_rules.update({"pedidos", "lotes", "impressao_rapida"})
        conn.execute(
...
CREATE TABLE IF NOT EXISTS production_recipes (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    product_stock_item_id INTEGER NOT NULL UNIQUE,
    nome TEXT NOT NULL,
    observacao TEXT,
    ativo INTEGER NOT NULL DEFAULT 1,
    criado_por INTEGER,
    criado_em TEXT NOT NULL,
    atualizado_em TEXT NOT NULL,
    FOREIGN KEY (product_stock_item_id) REFERENCES stock_items (id),
    FOREIGN KEY (criado_por) REFERENCES users (id)
);
...
CREATE TABLE IF NOT EXISTS production_recipe_items (
    ...
    UNIQUE(recipe_id, ingredient_stock_item_id)
);
...
CREATE TABLE IF NOT EXISTS production_order_logs (
    ...
    criado_em TEXT NOT NULL,
    FOREIGN KEY (order_id) REFERENCES production_orders (id) ON DELETE CASCADE,
    FOREIGN KEY (order_item_id) REFERENCES production_order_items (id) ON DELETE CASCADE,
    FOREIGN KEY (user_id) REFERENCES users (id)
);
...
def get_google_ai_api_key() -> str:
    return (get_setting("google_ai_api_key", "") or os.environ.get("GOOGLE_AI_API_KEY", "") or os.environ.get("GEMINI_API_KEY", "")).strip()


def normalizar_gemini_modelo(modelo: str) -> str:
    modelo = (modelo or "gemini-2.5-flash").strip()
    if modelo.startswith("models/"):
        modelo = modelo.split("/", 1)[1]
    return modelo or "gemini-2.5-flash"


def listar_modelos_gemini_disponiveis(api_key: str | None = None) -> list[dict[str, Any]]:
    """Lista modelos Gemini disponíveis para a chave e filtra os que aceitam generateContent."""
    api_key = (api_key or get_google_ai_api_key()).strip()
    if not api_key:
        return []
    ...
    for item in data.get("models", []) or []:
        ...
        modelos.append({
            "name": nome,
            "model": nome.split("/", 1)[1] if nome.startswith("models/") else nome,
            "displayName": item.get("displayName") or nome,
            "methods": metodos,
        })
    return modelos


def escolher_modelo_gemini_disponivel(modelo_preferido: str, api_key: str | None = None) -> str:
    preferido = normalizar_gemini_modelo(modelo_preferido)
    modelos = listar_modelos_gemini_disponiveis(api_key)
    if not modelos:
        return preferido
    disponiveis = {m["model"]: m for m in modelos}
    ...
    for nome in prioridades:
        if nome in disponiveis:
            return nome
    return modelos[0]["model"]


def chamar_gemini_simples(mensagem: str, *, system: str = "") -> str:
    api_key = get_google_ai_api_key()
    if not api_key:
        return "Configure sua chave do Google AI Studio/Gemini em Configurações para ativar a IA."
    modelo_config = normalizar_gemini_modelo(get_setting("google_ai_model", "gemini-2.5-flash"))
    modelo = escolher_modelo_gemini_disponivel(modelo_config, api_key)
    prompt = mensagem if not system else f"{system.strip()}\n\nPedido do usuário:\n{mensagem}"
    payload = {
        "contents": [
    ...
    try:
        ...
        if "not found" in msg.lower() or "not supported" in msg.lower():
            modelos = listar_modelos_gemini_disponiveis(api_key)
            sugestao = modelos[0]["model"] if modelos else "gemini-2.5-flash"
            return f"Erro ao chamar o Google AI Studio/Gemini: {msg}\nModelo configurado: {modelo_config}. Sugestão: salve o modelo {sugestao} em Configurações > Google AI Studio/Gemini."
        return f"Erro ao chamar o Google AI Studio/Gemini: {msg}"
    except Exception as exc:
        return f"Erro ao conectar com Google AI Studio/Gemini: {exc}"
    ...
    return "\n".join(partes).strip() or "O Gemini respondeu, mas não retornou texto."
...
def resetar_banco_mantendo_admin(conn: sqlite3.Connection, admin_user: sqlite3.Row) -> dict[str, int]:
    """Limpa dados operacionais e mantém apenas o admin que executou o reset."""
    admin_snapshot = dict(admin_user)
    admin_snapshot["role"] = "admin"
    admin_snapshot["permission_level"] = "admin"
    admin_snapshot["access_rules"] = serialize_access_rules(ACCESS_KEYS)
    admin_snapshot["ativo"] = 1
    admin_snapshot["store_id"] = None
    ...
    try:
        ...
        conn.execute(
            """
            INSERT INTO users (id, nome, username, password_hash, role, permission_level, access_rules, store_id, ativo, criado_em)
            VALUES (?, ?, ?, ?, 'admin', 'admin', ?, NULL, 1, ?)
            """,
            (
                int(admin_snapshot.get("id") or 1),
                admin_snapshot.get("nome") or "Administrador",
                admin_snapshot.get("username") or "admin",
                admin_snapshot.get("password_hash"),
                admin_snapshot.get("access_rules"),
                admin_snapshot.get("criado_em") or agora_iso(),
            ),
        )
        ...
        defaults = {
            ...
            "usar_conferente": "1",
            "maintenance_mode": "0",
            "code_editor_extra_password": "1",
            "impressora_padrao": "",
            "openai_model": "gpt-4.1-mini",
            "google_ai_model": "gemini-2.5-flash",
            "cloud_ai_provider": "openai",
        }
        # Evita que o estoque padrão seja recarregado automaticamente depois do reset.
        seed = carregar_seed_estoque()
        if isinstance(seed, dict) and seed.get("version"):
            defaults["stock_seed_version"] = str(seed.get("version"))
        for key, value in defaults.items():
            conn.execute("INSERT INTO settings (key, value) VALUES (?, ?)", (key, value))

        for nome_loja in [f"Loja {idx}" for idx in range(1, 21)] + ["CD"]:
            conn.execute("INSERT INTO stores (nome, ativo, criado_em) VALUES (?, 1, ?)", (nome_loja, agora_iso()))

        conn.execute(
            """
            INSERT INTO audit_logs (user_id, action, entity_type, entity_ref, details, created_at)
            VALUES (?, 'resetar_sistema', 'settings', 'reset_geral', ?, ?)
            """,
            (
                int(admin_snapshot.get("id") or 1),
                json.dumps({"tabelas_limpas": limpas, "mantido_apenas_admin": admin_snapshot.get("username")}, ensure_ascii=False),
                agora_iso(),
            ),
        )
        conn.commit()
...
def ensure_default_data() -> None:
    with closing(get_conn()) as conn:
        ...
        conn.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('impressora_padrao', '')")
        conn.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('offline_ai_model_id', '')")
        conn.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('offline_ai_enabled', '0')")
        # Garante a lista padrão completa de lojas do Alvorada.
        # Bancos antigos tinham apenas Loja 1 a Loja 5; na próxima abertura o sistema completa até a Loja 20.
        for nome_loja in [f"Loja {idx}" for idx in range(1, 21)] + ["CD"]:
            conn.execute("INSERT OR IGNORE INTO stores (nome, ativo, criado_em) VALUES (?, 1, ?)", (nome_loja, agora_iso()))
        conn.commit()
...
def usuarios() -> str | Response:
    if request.method == "POST":
        ...
        if role == "admin":
            ...
            # O cargo não marca permissões sozinho. Vale o que foi selecionado na tela,
            # removendo apenas áreas sensíveis, que são exclusivas do Admin real.
            if permission_level == "admin":
                permission_level = "comum"
            access_rules = sanitize_access_rules_for_role(access_rules, role)
    ...
    """)
    lojas = query_all("SELECT id, nome FROM stores WHERE ativo = 1 ORDER BY CASE WHEN nome = 'CD' THEN 9999 WHEN nome LIKE 'Loja %' THEN CAST(REPLACE(nome, 'Loja ', '') AS INTEGER) ELSE 5000 END, nome")
    return render_template(
        "usuarios.html",
        title="Usuários",
        users=users,
        ...
        default_access_by_role={key: sorted(value) for key, value in DEFAULT_ACCESS_BY_ROLE.items()},
        sensitive_access_keys=sorted(SENSITIVE_ACCESS_KEYS),
        no_report_access_roles=sorted(NO_REPORT_ACCESS_ROLES),
        report_access_key="relatorios",
        operational_access_keys=sorted(ACCESS_KEYS - SENSITIVE_ACCESS_KEYS),
        lojas=lojas,
    )
...
def lojas() -> str | Response:
    ...
    stores = query_all("SELECT * FROM stores ORDER BY ativo DESC, CASE WHEN nome = 'CD' THEN 9999 WHEN nome LIKE 'Loja %' THEN CAST(REPLACE(nome, 'Loja ', '') AS INTEGER) ELSE 5000 END, nome")
...
def configuracoes() -> str | Response:
    if request.method == "POST":
        ...
        set_setting("openai_model", (request.form.get("openai_model") or get_setting("openai_model", "gpt-4.1-mini")).strip())
        if "google_ai_model" in request.form:
            set_setting("google_ai_model", normalizar_gemini_modelo(request.form.get("google_ai_model") or "gemini-2.5-flash"))
        if "cloud_ai_provider" in request.form:
            provider = (request.form.get("cloud_ai_provider") or "openai").strip().lower()
            set_setting("cloud_ai_provider", "gemini" if provider == "gemini" else "openai")
        if request.form.get("offline_ai_form") == "1":
            set_setting("offline_ai_enabled", "1" if request.form.get("offline_ai_enabled") == "1" else "0")
            set_setting("offline_ai_model_id", (request.form.get("offline_ai_model_id") or "").strip())
        ...
        nova_google = (request.form.get("google_ai_api_key") or "").strip()
        if nova_google:
            set_setting("google_ai_api_key", nova_google)
        if request.form.get("limpar_google_ai_api_key") == "1":
            set_setting("google_ai_api_key", "")
        registrar_auditoria("salvar_configuracoes", "settings", "geral", {"maintenance_mode": request.form.get("maintenance_mode") == "1", "openai_configurada": bool(get_openai_api_key()), "gemini_configurada": bool(get_google_ai_api_key())})
    ...
    return render_template(
        ...
        google_ai_model=get_setting("google_ai_model", "gemini-2.5-flash"),
        google_ai_api_key_mask=mask_secret(get_google_ai_api_key()),
        google_ai_configurada=bool(get_google_ai_api_key()),
        cloud_ai_provider=get_setting("cloud_ai_provider", "openai"),
        modelos_gemini=listar_modelos_gemini_disponiveis(),
        modelos_offline=detectar_modelos_ia_locais(),
        offline_ai_model_id=get_setting("offline_ai_model_id", ""),
        offline_ai_enabled=get_setting("offline_ai_enabled", "0") == "1",
        offline_ai_conectada=bool(get_setting("offline_ai_model_id", "")) and get_setting("offline_ai_enabled", "0") == "1",
        mcp_db_path=DB_PATH,
        mcp_command="python -m controle_separacao.mcp_server",
    )


@app.post("/configuracoes/resetar-sistema")
@login_required
@module_required("configuracoes")
@roles_required("admin")
def resetar_sistema_configuracao() -> Response:
    senha_admin = request.form.get("senha_admin", "")
    confirmar = (request.form.get("confirmar_reset") or "").strip().upper()
    if not check_password_hash(g.user["password_hash"], senha_admin):
        flash("Senha do admin incorreta. O sistema não foi resetado.", "error")
        return redirect(url_for("configuracoes"))
    if confirmar != "RESETAR":
        flash("Digite RESETAR para confirmar a limpeza total do sistema.", "error")
        return redirect(url_for("configuracoes"))

    with closing(get_conn()) as conn:
        stats = resetar_banco_mantendo_admin(conn, g.user)

    session.clear()
    flash(f"Sistema resetado com sucesso. {stats.get('tabelas_limpas', 0)} tabelas foram limpas e apenas o admin foi mantido.", "success")
    return redirect(url_for("login"))


@app.get("/api/configuracoes/modelos-offline")
@login_required
@module_required("configuracoes")
@roles_required("admin")
def api_configuracoes_modelos_offline() -> Response:
    modelos = detectar_modelos_ia_locais()
    return jsonify({"ok": True, "modelos": modelos, "selecionado": get_setting("offline_ai_model_id", ""), "ativo": get_setting("offline_ai_enabled", "0") == "1"})


@app.post("/configuracoes/modelo-offline/testar")
@login_required
@module_required("configuracoes")
@roles_required("admin")
def testar_modelo_offline_configuracao() -> Response:
    modelo_id = (request.form.get("offline_ai_model_id") or get_setting("offline_ai_model_id", "")).strip()
    if not modelo_id:
        flash("Escolha um modelo offline antes de testar.", "error")
        return redirect(url_for("configuracoes"))
    try:
        resposta = gerar_resposta_com_modelo_local(modelo_id, "Responda apenas: IA offline Alvorada conectada.")
        ok = bool(resposta.strip())
        registrar_auditoria("testar_modelo_offline", "settings", "offline_ai", {"ok": ok, "modelo": modelo_id})
        flash(resposta[:500], "success" if ok else "error")
    except Exception as exc:
        registrar_auditoria("testar_modelo_offline", "settings", "offline_ai", {"ok": False, "modelo": modelo_id, "erro": str(exc)})
        flash(str(exc), "error")
    return redirect(url_for("configuracoes"))


@app.post("/api/agente/executar")
@login_required
def api_agente_executar() -> Response:
    data = request.get_json(silent=True) or {}
    texto = str(data.get("texto") or "").strip()
    if not texto.lower().startswith("/agente"):
        return jsonify({"ok": False, "erro": "Use /agente antes da pergunta."}), 400
    try:
        _, resposta = executar_comando_agente_no_texto(texto)
    except Exception as exc:
        return jsonify({"ok": False, "erro": str(exc)}), 500
    return jsonify({"ok": True, "resposta": resposta})


@app.post("/configuracoes/gemini/testar")
@login_required
@module_required("configuracoes")
@roles_required("admin")
def testar_gemini_configuracao() -> Response:
    resposta = chamar_gemini_simples("Responda apenas: IA Gemini Alvorada conectada.")
    ok = "erro" not in resposta.lower() and "configure" not in resposta.lower()
    registrar_auditoria("testar_gemini", "settings", "gemini", {"ok": ok, "modelo": get_setting("google_ai_model", "gemini-2.5-flash")})
    flash(resposta, "success" if ok else "error")
    return redirect(url_for("configuracoes"))


@app.get("/api/configuracoes/modelos-gemini")
@login_required
@module_required("configuracoes")
@roles_required("admin")
def api_configuracoes_modelos_gemini() -> Response:
    modelos = listar_modelos_gemini_disponiveis()
    return jsonify({"ok": True, "modelos": modelos, "selecionado": get_setting("google_ai_model", "gemini-2.5-flash")})
...
def api_ia_responder() -> Response:
    ...
    try:
        ...
        return jsonify({"ok": False, "resposta": str(exc)}), 500
    return jsonify({"ok": True, "resposta": resposta})
...
def importar_estoque_erp() -> str | Response:
    if not can_edit_stock_registration(g.user):
        return forbidden_redirect("Somente usuários com acesso operacional completo ou admin podem importar arquivo do ERP.")
    return render_template(
...
def detalhe_importacao_erp(import_id: int) -> str | Response:
    if not can_edit_stock_registration(g.user):
        return forbidden_redirect("Somente usuários com acesso operacional completo ou admin podem ver importações do ERP.")
...
def aplicar_importacao_erp(import_id: int) -> Response:
    if not can_edit_stock_registration(g.user):
        return forbidden_redirect("Somente usuários com acesso operacional completo ou admin podem aplicar importação do ERP.")
...
LOJAS_CNPJ_PDF = {
    "21414958000107": "1", "21414958000124": "2", "21414958000141": "3",
    "21414958000168": "4", "21414958000185": "5", "21414958000202": "6",
    "21414958000229": "7", "21414958000246": "8", "21414958000263": "9",
    "21414958000280": "10", "21414958000307": "11", "21414958000324": "12",
    "21414958000341": "13", "21414958000368": "14", "21414958000385": "15",
    "21414958000402": "16", "21414958000429": "17", "21414958000446": "18",
    "21414958000463": "19", "21414958000480": "20", "21414958001090": "6",
    "21414958001170": "7", "21414958001766": "CD",
}

LOJAS_RAZAO_SOCIAL_PDF = {
    "SANTO ANTONIO SGC": "18", "STO ANTONIO SGC": "18", "STO. ANTONIO SGC": "18",
    "SANTO ANTONIO": "18", "STO ANTONIO": "18", "STO. ANTONIO": "18",
    "ANTONIO MACHADO": "8", "ANTÔNIO MACHADO": "8", "8 - MACHADO": "8", "MACHADO": "8",
    "COMERCIO": "1", "COM.": "1", "COM": "1", "GUARDA": "2", "CESARIO": "3", "SILVIANO": "4",
    "VICENTE": "5", "GETULIO": "6", "GETÚLIO": "6", "CAPITAO": "7", "CAPITÃO": "7",
    "ANTONIO": "8", "ANTÔNIO": "8", "JOSE": "9", "JOSÉ": "9", "PRINCESA": "10", "WENCESLAU": "11",
    "FOCH": "12", "ALAGOAS": "13", "GOVERNADOR": "14", "VALADARES": "14", "JOAO OLIVO": "15",
    "JOÃO OLIVO": "15", "MONSIEUR REBOURGEON": "16", "REBOURGEON": "16",
    "PADRE LOURENCO": "17", "PADRE LOURENÇO": "17", "PLINIO SALGADO": "19", "IMIGRANTES": "20",
}


def _normalizar_texto_loja(valor: str) -> str:
    import unicodedata
    valor = unicodedata.normalize("NFD", str(valor or ""))
    valor = "".join(ch for ch in valor if unicodedata.category(ch) != "Mn")
    valor = re.sub(r"[^A-Za-z0-9]+", " ", valor.upper())
    return re.sub(r"\s+", " ", valor).strip()


def _nome_loja_padrao(numero: str) -> str:
    numero = str(numero or "").strip().upper()
    if numero == "CD":
        return "CD"
    return f"Loja {int(numero)}" if numero.isdigit() else numero


def _detectar_loja_pdf(texto: str) -> dict[str, str]:
    """Detecta a loja do PDF por CNPJ ou por razão social/apelido no cabeçalho."""
    texto = texto or ""
    somente_digitos = re.sub(r"\D", "", texto)
    for cnpj, numero in LOJAS_CNPJ_PDF.items():
        if cnpj in somente_digitos:
            return {"numero": numero, "nome": _nome_loja_padrao(numero), "motivo": f"CNPJ {cnpj}"}

    # A razão social da loja normalmente fica nas primeiras linhas do PDF.
    cabecalho = "\n".join(texto.splitlines()[:12]) or texto[:800]
    candidatos = [cabecalho]
    m = re.search(r"(?im)^\s*(.+?)\s*\(([^()]+)\)", cabecalho)
    if m:
        candidatos.extend([m.group(1), m.group(2)])
    normalizado = " | ".join(_normalizar_texto_loja(c) for c in candidatos if c)

    for chave, numero in sorted(LOJAS_RAZAO_SOCIAL_PDF.items(), key=lambda kv: len(_normalizar_texto_loja(kv[0])), reverse=True):
        chave_norm = _normalizar_texto_loja(chave)
        if not chave_norm:
            continue
        if re.search(rf"(^|\s){re.escape(chave_norm)}($|\s|\|)", normalizado):
            return {"numero": numero, "nome": _nome_loja_padrao(numero), "motivo": chave}
    return {"numero": "", "nome": "", "motivo": ""}

def extrair_texto_pdf_padrao(file_storage: Any) -> str:
    """Extrai texto de um PDF de pedido/separação usando pypdf.

    A importação é tolerante: se o PDF for escaneado/imagem, o texto virá vazio
    e a tela avisará o usuário para usar um PDF textual.
    """
    try:
        from pypdf import PdfReader
    except Exception as exc:
        raise RuntimeError("Instale a dependência pypdf para importar PDF: pip install pypdf") from exc
    ...
    for page in reader.pages:
        try:
            ...
            continue
    # Usa quebra de página para permitir detectar PDFs com uma loja/pedido por página.
    texto = "\n\f\n".join(partes).strip()
    if not texto:
        raise ValueError("Não consegui ler texto desse PDF. Ele pode estar escaneado como imagem.")
    return texto


def _pdf_capturar_campo(texto: str, labels: list[str]) -> str:
    for label in labels:
        padrao = rf"(?im)^\s*{re.escape(label)}\s*[:\-]?\s*(.+?)\s*$"
        match = re.search(padrao, texto)
        if match:
            return match.group(1).strip()[:120]
        padrao_inline = rf"(?i)\b{re.escape(label)}\s*[:\-]\s*([^\n\r]+)"
        match = re.search(padrao_inline, texto)
        if match:
            return match.group(1).strip()[:120]
    return ""


def _numero_para_loja_pdf(numero: str) -> dict[str, str]:
    numero = str(numero or "").strip().upper()
    if not numero:
        return {"numero": "", "nome": "", "motivo": ""}
    if numero.startswith("MR"):
        numero = numero[2:]
    numero = numero.lstrip("0") or "0"
    if numero.isdigit() and 1 <= int(numero) <= 20:
        return {"numero": str(int(numero)), "nome": _nome_loja_padrao(str(int(numero))), "motivo": f"MR{int(numero):02d}"}
    return {"numero": "", "nome": "", "motivo": ""}


def _parse_decimal_pdf(valor: str, default: float = 0.0) -> float:
    try:
        return parse_float(str(valor or "").replace(".", "").replace(",", "."), "Número")
    except Exception:
        try:
            return parse_float(valor, "Número")
        except Exception:
            return default


def _detectar_pedido_lista_reabastecimento(texto: str) -> str:
    texto = texto or ""
    m = re.search(r"(?i)\bN[ÚU]MERO\s+([0-9]+(?:/[0-9]+)?)", texto)
    if m:
        return m.group(1).strip()
    # Alguns PDFs extraem o cabeçalho fora de ordem, ficando algo como 04/05/20261429384/26Pag.
    m = re.search(r"\b(\d{6,8}/\d{2})\b", texto[:700])
    if m:
        return m.group(1).strip()
    m = re.search(r"(?i)(?:N[ÚU]MERO|PEDIDO).*?(\d{6,8}(?:/\d{2})?)", texto[:700])
    if m:
        return m.group(1).strip()
    return _pdf_capturar_campo(texto, ["Pedido número", "Pedido numero", "Pedido", "Nº Pedido", "Numero Pedido", "Número Pedido"])


def _analisar_lista_reabastecimento_por_paginas(texto: str) -> dict[str, Any] | None:
    """Lê o modelo 'LISTA DE REABASTECIMENTO' com uma loja por página.

    Exemplo do cabeçalho: MR02 MAGLIONI RIBEIRO & CIA LTDA (GUARDA) ... NÚMERO 1429384/26.
    Cada linha de item costuma terminar com: estoque, unidade, qtd pedido, embalagem e fator.
    """
    if not re.search(r"(?i)LISTA\s+DE\s+REABASTECIMENTO", texto or ""):
        return None
    paginas = [p.strip() for p in re.split(r"\f+", texto) if p.strip()] or [texto]
    itens: list[dict[str, Any]] = []
    pedidos: list[str] = []
    lojas_detectadas: dict[str, str] = {}
    vistos: set[tuple[str, str, str, float]] = set()

    for pagina in paginas:
        # Detecta loja primeiro pelo código MRxx; se não houver, usa a lógica por apelido/CNPJ.
        loja_detectada = {"numero": "", "nome": "", "motivo": ""}
        m_loja = re.search(r"(?im)^\s*MR\s*0*([0-9]{1,2})\b", pagina)
        if not m_loja:
            m_loja = re.search(r"(?i)\bMR\s*0*([0-9]{1,2})\b", pagina[:500])
        if m_loja:
            loja_detectada = _numero_para_loja_pdf(m_loja.group(1))
        if not loja_detectada.get("numero"):
            loja_detectada = _detectar_loja_pdf(pagina)
        loja_numero = loja_detectada.get("numero", "")
        loja_nome = loja_detectada.get("nome", "")
        if loja_numero and loja_nome:
            lojas_detectadas[loja_numero] = loja_nome

        pedido_pagina = _detectar_pedido_lista_reabastecimento(pagina)
        if pedido_pagina and pedido_pagina not in pedidos:
            pedidos.append(pedido_pagina)

        linhas = [re.sub(r"\s+", " ", l).strip() for l in pagina.splitlines()]
        for linha in linhas:
            if not linha or len(linha) < 25:
                continue
            if re.search(r"(?i)\b(LISTA|MAGLIONI|PAG\.?|N[ÚU]MERO|C[ÓO]DIGO|COD BARRAS|DESCRI[ÇC][ÃA]O|ENDERE[ÇC]O|ESTOQUE|QTD|SEPARADOR|DATA|HORA|GRANDEZA)\b", linha):
                continue
            # Linha comum: 223449 7898963745 HAMBURGUER QUALITY 51-000-00- 576,00 UN 1,00 CX 36,00
            m = re.match(
                r"^(?P<codigo>\d{3,12})\s+(?P<cod_barras>\d{8,14})\s+(?P<descricao>.+?)\s+(?P<endereco>\d{2,3}-\d{3}-\d{2,3}-?)\s+(?P<estoque>[0-9\.]+,[0-9]{2})\s*(?P<unidade>[A-Z]{1,4})?\s+(?P<qtd>[0-9\.]+,[0-9]{2})\s+(?P<emb>[A-Z]{1,4})\s+(?P<fator>[0-9\.]+,[0-9]{2})$",
                linha,
                re.IGNORECASE,
            )
            if not m:
                # Variação tolerante sem endereço bem formado.
                m = re.match(
                    r"^(?P<codigo>\d{3,12})\s+(?P<cod_barras>\d{8,14})\s+(?P<descricao>.+?)\s+(?P<estoque>[0-9\.]+,[0-9]{2})\s*(?P<unidade>[A-Z]{1,4})?\s+(?P<qtd>[0-9\.]+,[0-9]{2})\s+(?P<emb>[A-Z]{1,4})\s+(?P<fator>[0-9\.]+,[0-9]{2})$",
                    linha,
                    re.IGNORECASE,
                )
            if not m:
            ...
            if len(descricao) < 3:
                continue
            qtd = _parse_decimal_pdf(m.group("qtd"), 0.0)
            fator = _parse_decimal_pdf(m.group("fator"), 1.0) or 1.0
            chave = (loja_numero, codigo, descricao.casefold(), qtd)
            if chave in vistos:
                continue
            vistos.add(chave)
            itens.append({
                ...
                "fator_embalagem": fator,
                "quantidade": qtd,
                "custo_unitario_ref": 0.0,
                "loja_numero": loja_numero,
                "loja_nome": loja_nome,
                "pedido_origem": pedido_pagina,
                "cod_barras": m.group("cod_barras").strip(),
                "embalagem_sigla": m.group("emb").upper(),
            })
    ...
    if len(pedidos) == 1:
        ...
        pedido = f"Vários ({len(pedidos)} pedidos)"
    else:
        pedido = ""
    if len(lojas_detectadas) == 1:
        loja = next(iter(lojas_detectadas.values()))
        loja_numero = next(iter(lojas_detectadas.keys()))
        loja_nome = loja
    elif lojas_detectadas:
        loja = f"Múltiplas lojas ({len(lojas_detectadas)})"
        loja_numero = ""
        loja_nome = ""
    else:
        loja = ""
    ...
    return {
        ...
        "loja_numero": loja_numero,
        "loja_nome": loja_nome,
        "loja_motivo": "MRxx" if lojas_detectadas else "",
        "lojas_detectadas": lojas_detectadas,
        "data_doc": _pdf_capturar_campo(texto, ["Data", "Em .", "Em"]),
        "itens": itens,
        "linhas_lidas": len([l for l in texto.splitlines() if l.strip()]),
        "modelo_pdf": "lista_reabastecimento_multiloja",
    }


def analisar_pdf_pedido_padrao(texto: str) -> dict[str, Any]:
    """Transforma texto de PDF em cabeçalho + linhas editáveis.

    Como o layout do PDF pode variar, a regra procura linhas com código, descrição
    e quantidade no final. O usuário sempre revisa/edita antes de criar ou exportar.
    """
    modelo_multiloja = _analisar_lista_reabastecimento_por_paginas(texto)
    if modelo_multiloja:
        return modelo_multiloja

    pedido = _pdf_capturar_campo(texto, ["Pedido", "Pedido número", "Pedido numero", "Nº Pedido", "Numero Pedido", "Número Pedido", "Documento", "Orçamento"])
    loja = _pdf_capturar_campo(texto, ["Loja", "Cliente", "Destino", "Filial"])
    data_doc = _pdf_capturar_campo(texto, ["Data", "Emissão", "Data Emissão", "Data do Pedido"])
    loja_detectada = _detectar_loja_pdf(texto)
    if loja_detectada.get("nome"):
        loja = loja_detectada["nome"]

    itens: list[dict[str, Any]] = []
    vistos: set[tuple[str, str, float]] = set()
    linhas = [re.sub(r"\s+", " ", l).strip() for l in texto.splitlines()]
    ignorar = re.compile(r"(?i)\b(total|subtotal|valor|cliente|loja|pedido|documento|data|emiss|cnpj|cpf|fone|endereço|endereco|cep|vendedor|observa)\b")

    for linha in linhas:
        if len(linha) < 8 or ignorar.search(linha):
            continue
        # Formatos comuns: CODIGO DESCRICAO QTD | CODIGO DESCRICAO EMB12 QTD | CODIGO DESCRICAO UN QTD
        match = re.match(
            r"^([A-Za-z0-9\.\-\/]{2,30})\s+(.+?)\s+(?:UN|UND|UNID|CX|KG|PC|PCT|EMB\s*\d+[\.,]?\d*\s+)?([0-9]+(?:[\.,][0-9]+)?)$",
            linha,
            re.IGNORECASE,
        )
        if not match:
            continue
        codigo, descricao, qtd_raw = match.groups()
        descricao = descricao.strip(" -–—\t")
        if len(descricao) < 3 or not re.search(r"[A-Za-zÀ-ÿ]", descricao):
            continue
        try:
            quantidade = parse_float(qtd_raw, "Quantidade")
        except ValueError:
            continue
        fator = 1.0
        fator_match = re.search(r"(?i)\bemb\s*([0-9]+(?:[\.,][0-9]+)?)", linha)
        if fator_match:
            try:
                fator = parse_float(fator_match.group(1), "Fator da embalagem") or 1.0
            except ValueError:
                fator = 1.0
        chave = (codigo.strip(), descricao.casefold(), quantidade)
        if chave in vistos:
            continue
        vistos.add(chave)
        itens.append({
            "codigo": codigo.strip()[:60],
            "descricao": descricao[:220],
            "fator_embalagem": fator,
            "quantidade": quantidade,
            "custo_unitario_ref": 0.0,
    ...
    return {
        ...
        "loja_numero": loja_detectada.get("numero", ""),
        "loja_nome": loja_detectada.get("nome", ""),
        "loja_motivo": loja_detectada.get("motivo", ""),
        "lojas_detectadas": {loja_detectada.get("numero", ""): loja_detectada.get("nome", "")} if loja_detectada.get("numero") else {},
        "data_doc": data_doc,
        "itens": itens,
        "linhas_lidas": len([l for l in linhas if l]),
    }


def _itens_pdf_import_form() -> list[dict[str, Any]]:
    codigos = request.form.getlist("codigo[]")
    descricoes = request.form.getlist("descricao[]")
    fatores = request.form.getlist("fator_embalagem[]")
    quantidades = request.form.getlist("quantidade[]")
    custos = request.form.getlist("custo_unitario_ref[]")
    lojas_item = request.form.getlist("loja_numero_item[]")
    lojas_nome_item = request.form.getlist("loja_nome_item[]")
    pedidos_item = request.form.getlist("pedido_item[]")
    itens: list[dict[str, Any]] = []
    total = max(len(codigos), len(descricoes), len(quantidades))
    for idx in range(total):
        ...
        if not codigo and not descricao and not str(qtd_raw).strip():
            continue
        if not codigo or not descricao:
            raise ValueError("Todas as linhas precisam ter código e descrição.")
        quantidade = parse_float(str(qtd_raw), "Quantidade")
        if quantidade <= 0:
            continue
        fator = parse_fator_embalagem(fatores[idx] if idx < len(fatores) else "1", default=1.0)
        custo = parse_float(str(custos[idx] if idx < len(custos) else "0"), "Custo unitário") if (idx < len(custos) and str(custos[idx]).strip()) else 0.0
        itens.append({
            "codigo": codigo[:60],
            "descricao": descricao[:220],
            "fator_embalagem": fator,
            "quantidade": quantidade,
            "custo_unitario_ref": custo,
            "loja_numero": (lojas_item[idx] if idx < len(lojas_item) else "").strip(),
            "loja_nome": (lojas_nome_item[idx] if idx < len(lojas_nome_item) else "").strip(),
            "pedido_origem": (pedidos_item[idx] if idx < len(pedidos_item) else "").strip(),
        })
    if not itens:
        raise ValueError("Nenhum item válido para continuar.")
    return itens


def gerar_excel_importacao_pdf(itens: list[dict[str, Any]], meta: dict[str, Any]) -> Response:
    try:
        from openpyxl import Workbook
        from openpyxl.styles import Font, PatternFill
    except ImportError:
        flash("Para exportar em Excel, instale a dependência openpyxl.", "error")
        return redirect(url_for("importar_pdf_pedido"))

    wb = Workbook()
    ws = wb.active
    ws.title = "PDF Importado"
    ws.append(["Campo", "Valor"])
    ws.append(["Pedido", meta.get("pedido", "")])
    ws.append(["Loja/Cliente", meta.get("loja", "")])
    ws.append(["Data do PDF", meta.get("data_doc", "")])
    ws.append(["Exportado em", agora_br()])
    ws.append([])
    headers = ["Loja", "Pedido origem", "Código", "Descrição", "Fator embalagem", "Quantidade", "Custo unitário", "Total"]
    ws.append(headers)
    header_row = ws.max_row
    for cell in ws[header_row]:
        cell.font = Font(bold=True, color="FFFFFF")
        cell.fill = PatternFill("solid", fgColor="2F6B2F")
    for item in itens:
        qtd = float(item.get("quantidade") or 0)
        custo = float(item.get("custo_unitario_ref") or 0)
        ws.append([item.get("loja_nome") or item.get("loja_numero", ""), item.get("pedido_origem", ""), item.get("codigo", ""), item.get("descricao", ""), item.get("fator_embalagem", 1), qtd, custo, qtd * custo])
    for col, width in {"A": 16, "B": 18, "C": 18, "D": 48, "E": 18, "F": 14, "G": 16, "H": 16}.items():
        ws.column_dimensions[col].width = width
    buffer = io.BytesIO()
    wb.save(buffer)
    buffer.seek(0)
    filename = f"importacao_pdf_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    return send_file(buffer, as_attachment=True, download_name=filename, mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")


@app.route("/separacoes/importar-pdf", methods=["GET", "POST"])
@login_required
@module_required("pedidos")
def importar_pdf_pedido() -> str | Response:
    preview = None
    if request.method == "POST":
        ...
        if action == "preview":
            ...
            if not pdf_file.filename.lower().endswith(".pdf"):
                ...
                return redirect(url_for("importar_pdf_pedido"))
            try:
                texto = extrair_texto_pdf_padrao(pdf_file)
                preview = analisar_pdf_pedido_padrao(texto)
                if not preview["itens"]:
                    flash("Li o PDF, mas não encontrei itens automaticamente. Você pode adicionar as linhas manualmente na prévia.", "warning")
                else:
                    flash(f"PDF lido com {len(preview['itens'])} item(ns). Revise os campos antes de continuar.", "success")
            except Exception as exc:
                flash(str(exc), "error")
                preview = {"pedido": "", "loja": "", "loja_numero": "", "loja_nome": "", "loja_motivo": "", "lojas_detectadas": {}, "data_doc": "", "itens": [], "linhas_lidas": 0}
        else:
            meta = {
                "pedido": request.form.get("pedido", "").strip(),
                "loja": request.form.get("loja_pdf", "").strip(),
                "loja_numero": request.form.get("loja_numero", "").strip(),
                "loja_nome": request.form.get("loja_nome", "").strip(),
                "loja_motivo": request.form.get("loja_motivo", "").strip(),
                "data_doc": request.form.get("data_doc", "").strip(),
            }
            try:
                itens = _itens_pdf_import_form()
            except ValueError as exc:
                flash(str(exc), "error")
                preview = {**meta, "itens": [], "linhas_lidas": 0}
            else:
                if action == "export_excel":
                    registrar_auditoria("exportar_importacao_pdf_excel", "pdf_import", meta.get("pedido", ""), {"total_itens": len(itens)})
                    return gerar_excel_importacao_pdf(itens, meta)
                if action == "criar_separacao":
                    lote_nome = request.form.get("lote_nome", "").strip() or f"PDF {meta.get('pedido') or datetime.now().strftime('%d/%m/%Y')}"
                    data_referencia = request.form.get("data_referencia", "").strip() or datetime.now().strftime("%Y-%m-%d")
                    ...
                    observacao = request.form.get("observacao", "").strip()
                    itens_com_loja = [item for item in itens if str(item.get("loja_numero") or "").strip()]
                    if not responsavel_id.isdigit() or (not stores and not itens_com_loja):
                        flash("Para criar a separação, selecione responsável e ao menos uma loja. Em PDFs multiloja, a loja pode vir detectada em cada página.", "error")
                        preview = {**meta, "itens": itens, "linhas_lidas": 0}
                    else:
                        lote_codigo = novo_lote_codigo()
                        criadas = 0
                        with closing(get_conn()) as conn:
                            lojas_por_numero = {}
                            for row in conn.execute("SELECT id, nome FROM stores WHERE ativo = 1").fetchall():
                                nome = row["nome"]
                                if nome == "CD":
                                    lojas_por_numero["CD"] = int(row["id"])
                                m_nome = re.match(r"Loja\s+(\d+)$", str(nome), re.IGNORECASE)
                                if m_nome:
                                    lojas_por_numero[str(int(m_nome.group(1)))] = int(row["id"])
                            grupos: dict[int, list[dict[str, Any]]] = {}
                            if itens_com_loja:
                                for item in itens:
                                    numero = str(item.get("loja_numero") or "").strip().upper()
                                    store_id = lojas_por_numero.get(str(int(numero)) if numero.isdigit() else numero) if numero else None
                                    if store_id:
                                        grupos.setdefault(store_id, []).append(item)
                            else:
                                for store_id_raw in stores:
                                    if str(store_id_raw).isdigit():
                                        grupos[int(store_id_raw)] = list(itens)
                            if not grupos:
                                flash("Não consegui relacionar as lojas detectadas do PDF com as lojas cadastradas. Confira se Loja 1 a Loja 20 existem e estão ativas.", "error")
                                preview = {**meta, "itens": itens, "linhas_lidas": 0}
                                return render_template(
                                    "importar_pdf_pedido.html",
                                    title="Importar PDF padrão",
                                    hoje=datetime.now().strftime("%Y-%m-%d"),
                                    preview=preview,
                                    stores=query_all("SELECT * FROM stores WHERE ativo = 1 ORDER BY CASE WHEN nome = 'CD' THEN 9999 WHEN nome LIKE 'Loja %' THEN CAST(REPLACE(nome, 'Loja ', '') AS INTEGER) ELSE 5000 END, nome"),
                                    separadores=usuarios_por_role("separador"),
                                    conferentes=usuarios_por_role("conferente"),
                                    usar_conferente=get_setting("usar_conferente", "1") == "1",
                                )
                            for store_id, itens_da_loja in grupos.items():
                                cursor = conn.execute(
                                    """
                                    ...
                                    (
                                        ...
                                        int(store_id),
                                        int(responsavel_id),
                                        int(conferente_id) if usar_conferente and conferente_id and str(conferente_id).isdigit() else None,
                                        usar_estoque,
                                        (observacao + "\n" if observacao else "") + f"Criado por importação de PDF. Pedido: {meta.get('pedido') or '-'}",
                                        g.user["id"],
                                        agora_iso(),
                                    ),
                                )
                                separation_id = cursor.lastrowid
                                criadas += 1
                                for item in itens_da_loja:
                                    conn.execute(
                                        """
                                        INSERT INTO separation_items (
                                            separation_id, codigo, descricao, fator_embalagem, quantidade_pedida,
                                            quantidade_separada, status, custo_unitario_ref, criado_em, atualizado_em
                                        ) VALUES (?, ?, ?, ?, ?, 0, 'PENDENTE', ?, ?, ?)
                                        """,
                                        (
                                            separation_id,
                                            item["codigo"],
                                            item["descricao"],
                                            float(item.get("fator_embalagem") or 1),
                                            float(item.get("quantidade") or 0),
                                            float(item.get("custo_unitario_ref") or 0),
                                            agora_iso(),
                                            agora_iso(),
                                        ),
                                    )
                            conn.commit()
                        registrar_auditoria("criar_separacao_pdf", "separations", lote_codigo, {"pedido": meta.get("pedido", ""), "total_itens": len(itens), "lojas": criadas})
                        flash(f"Separação criada a partir do PDF com {criadas} loja(s). Revise o lote antes de iniciar a operação.", "success")
...
def listar_impressoras_disponiveis() -> list[str]:
    """Lista impressoras visíveis para o computador/servidor onde o sistema está rodando."""
    nomes: list[str] = []
    sistema = platform.system().lower()
    if sistema == "windows":
        try:
            import win32print  # type: ignore
            flags = win32print.PRINTER_ENUM_LOCAL | win32print.PRINTER_ENUM_CONNECTIONS | win32print.PRINTER_ENUM_NETWORK
            for printer in win32print.EnumPrinters(flags):
                if len(printer) >= 3 and printer[2]:
                    nomes.append(str(printer[2]))
        except Exception:
            # Sem pywin32, tenta WMIC/PowerShell em instalações Windows comuns.
            for cmd in (["powershell", "-NoProfile", "-Command", "Get-Printer | Select-Object -ExpandProperty Name"], ["wmic", "printer", "get", "name"]):
                try:
...
def _buscar_produtos_impressao(termo: str, limite: int = 30) -> list[sqlite3.Row]:
    ...
    return query_all(
        """
        SELECT id, codigo, codigo_barras, descricao, fator_embalagem, quantidade_atual, custo_unitario
        FROM stock_items
        WHERE ativo = 1 AND (codigo = ? OR codigo_barras = ? OR codigo LIKE ? OR codigo_barras LIKE ? OR descricao LIKE ?)
        ORDER BY
            CASE
                WHEN codigo = ? THEN 0
                WHEN codigo_barras = ? THEN 1
        ...
        LIMIT ?
        """,
        (termo, termo, like, like, like, termo, termo, like, max(1, min(int(limite or 30), 80))),
    )


def _produto_impressao_por_id(stock_item_id: str) -> sqlite3.Row | None:
    if not str(stock_item_id or "").isdigit():
        return None
    return query_one(
        """
        SELECT id, codigo, codigo_barras, descricao, fator_embalagem, quantidade_atual, custo_unitario
...
def _montar_conteudo_qrcode(payload: dict[str, Any]) -> str:
    produto = str(payload.get("produto") or "").strip()
    codigo = str(payload.get("codigo") or "").strip()
    peso = str(payload.get("peso") or "").strip()
    validade = str(payload.get("validade") or "").strip()
    etiqueta = str(payload.get("etiqueta") or "").strip()
    partes = []
    if produto:
    ...
    if peso:
        partes.append(f"Peso/Emb.: {peso}")
    if validade:
        partes.append(f"Validade: {validade}")
    if etiqueta:
        partes.append(f"Etiqueta: {etiqueta}")
    return "\n".join(partes) or etiqueta or produto or codigo


def _gerar_qrcode_png_bytes(conteudo: str) -> io.BytesIO:
    try:
        import qrcode
    except ImportError as exc:
        raise RuntimeError("Instale a dependência qrcode[pil] para gerar QR Code na etiqueta.") from exc
    img = qrcode.make(conteudo or "Produto Alvorada")
    buffer = io.BytesIO()
...
def _preencher_base_impressao(payload: dict[str, Any]) -> io.BytesIO:
    try:
        from openpyxl import load_workbook
        from openpyxl.drawing.image import Image as XLImage
        from openpyxl.styles import Alignment, Font
    except ImportError as exc:
        raise RuntimeError("Instale a dependência openpyxl para gerar a planilha de impressão.") from exc
    if not os.path.exists(PRINT_TEMPLATE_PATH):
        raise RuntimeError("Modelo Base.xlsx não encontrado na pasta templates_impressao.")

    wb = load_workbook(PRINT_TEMPLATE_PATH)
    ws = wb.active
    produto = str(payload.get("produto") or "").strip().upper()
    peso = str(payload.get("peso") or "").strip().upper()
    data_producao = str(payload.get("data_producao") or "").strip()
    validade = str(payload.get("validade") or "").strip()
    etiqueta = str(payload.get("etiqueta") or "").strip()
    ...
    ws["D8"] = validade
    ws["E8"] = etiqueta
    # A9:C10 é o título "QUANT. DE PACOTES"; A11:C11 é o espaço livre do modelo para a quantidade.
    ws["A11"] = quantidade
    if codigo:
        ws["D12"] = "CÓDIGO"
        ws["E12"] = codigo

    if etiqueta_tipo == "qrcode":
        conteudo_qr = _montar_conteudo_qrcode(payload)
        qr_buffer = _gerar_qrcode_png_bytes(conteudo_qr)
...
def _imprimir_planilha(buffer: io.BytesIO, impressora: str = "") -> tuple[bool, str]:
    temp_dir = tempfile.mkdtemp(prefix="alvorada_print_")
    caminho = os.path.join(temp_dir, f"etiqueta_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx")
    ...
    try:
        if sistema == "windows":
            try:
                ...
                if impressora:
                    try:
                        antiga = win32print.GetDefaultPrinter()
                ...
                if antiga:
                    try:
                        win32print.SetDefaultPrinter(antiga)
                    except Exception:
                        pass
                return True, "Enviado para a fila de impressão do Windows."
            except Exception:
                try:
                    os.startfile(caminho, "print")  # type: ignore[attr-defined]
                    return True, "Enviado para a impressora padrão do Windows."
                except Exception as exc:
                    return False, f"Não consegui imprimir automaticamente no Windows: {exc}"
        else:
            cmd = ["lp"]
            if impressora:
            ...
            return True, "Enviado para a fila de impressão."
    except Exception as exc:
        return False, f"Não consegui enviar para impressão automática: {exc}"


@app.route("/impressao-rapida", methods=["GET", "POST"])
@login_required
@module_required("impressao_rapida")
def impressao_rapida() -> str | Response:
    q = request.values.get("q", "").strip()
    produtos = _buscar_produtos_impressao(q) if q else []
    impressoras = listar_impressoras_disponiveis()
    impressora_padrao = get_setting("impressora_padrao", "")
    form = {
        "produto": "",
        "codigo": "",
        "peso": "",
        "data_producao": datetime.now().strftime("%d/%m/%Y"),
        "validade": "",
        "etiqueta": "",
        "etiqueta_tipo": "texto",
        "quantidade_pacotes": "",
    ...
    produto_escolhido = _produto_impressao_por_id(stock_item_id)
    if produto_escolhido:
        form.update({
            "produto": produto_escolhido["descricao"] or "",
            "codigo": produto_escolhido["codigo"] or produto_escolhido["codigo_barras"] or "",
            "peso": fmt_fator_embalagem(produto_escolhido["fator_embalagem"] or 1).replace("Emb", ""),
        })

    if request.method == "POST":
        action = request.form.get("action", "baixar")
        for key in form.keys():
            form[key] = request.form.get(key, form.get(key, "")).strip()
        if not form["produto"]:
            flash("Informe ou selecione um produto para imprimir.", "error")
        else:
            try:
                buffer = _preencher_base_impressao(form)
            except Exception as exc:
                flash(str(exc), "error")
            else:
                ...
                if action == "visualizar":
                    ...
                    ok, msg = _imprimir_planilha(buffer, form.get("impressora", ""))
                    flash(msg, "success" if ok else "error")
                    if ok:
                        return redirect(url_for("impressao_rapida", q=q, stock_item_id=stock_item_id))
                else:
                    filename = f"etiqueta_alvorada_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
                    return send_file(buffer, as_attachment=True, download_name=filename, mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

    return render_template(
        "impressao_rapida.html",
        ...
        servidor_sistema=platform.system(),
        qr_preview_text=_montar_conteudo_qrcode(form),
    )


@app.post("/impressao-rapida/config")
@login_required
@module_required("impressao_rapida")
def impressao_rapida_config() -> Response:
    impressora = request.form.get("impressora_padrao", "").strip()
    with closing(get_conn()) as conn:
        conn.execute("INSERT INTO settings (key, value) VALUES ('impressora_padrao', ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value", (impressora,))
        conn.commit()
    registrar_auditoria("configurar_impressora_padrao", "settings", "impressora_padrao", {"impressora": impressora})
    flash("Impressora padrão da impressão rápida salva.", "success")
    return redirect(url_for("impressao_rapida"))


@app.get("/api/impressao/impressoras")
@login_required
@module_required("impressao_rapida")
def api_impressoras() -> Response:
    return jsonify({"ok": True, "impressoras": listar_impressoras_disponiveis(), "padrao": get_setting("impressora_padrao", "")})
...
def _loja_codigo_para_sistema_alvorada(store_nome: Any, store_id: Any = None) -> str:
    """Converte o nome da loja para o código usado no sistema_alvorada.

    Quando a loja estiver cadastrada como "Loja 8", exporta "8". Se não houver
    número no nome, usa o id interno como fallback para manter o arquivo válido.
    """
    texto = str(store_nome or "").strip()
    match = re.search(r"\d+", texto)
    if match:
        return match.group(0)
    return str(store_id or texto or "0")


def criar_schema_sistema_alvorada(conn: sqlite3.Connection) -> None:
    conn.executescript(
        """
        CREATE TABLE IF NOT EXISTS pedidos_suspensos (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            loja TEXT,
            produto TEXT,
            nome_produto TEXT,
            ...
            qtd_lancada REAL DEFAULT 0,
            ultima_execucao TEXT,
            mensagem_execucao TEXT
        );
        ...
        CREATE TABLE IF NOT EXISTS produtos_editados (
            item TEXT PRIMARY KEY,
            descricao TEXT NOT NULL,
            embalagem REAL DEFAULT 1,
            categoria TEXT,
            editado_em TEXT DEFAULT CURRENT_TIMESTAMP
        );
        CREATE TABLE IF NOT EXISTS produtos_removidos (
            item TEXT PRIMARY KEY,
            removido_em TEXT DEFAULT CURRENT_TIMESTAMP
        );
        CREATE TABLE IF NOT EXISTS execucoes_lote (
            ...
            lojas_previstas INTEGER DEFAULT 0,
            lojas_concluidas INTEGER DEFAULT 0,
            lojas_com_erro INTEGER DEFAULT 0,
            mensagem TEXT
        );
        CREATE TABLE IF NOT EXISTS execucoes_lote_lojas (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            codigo_site TEXT,
            nota_codigo TEXT,
            item_pausa_codigo TEXT,
            item_pausa_nome TEXT,
            item_pausa_indice INTEGER DEFAULT 0,
            status TEXT DEFAULT 'PENDENTE',
            itens_previstos INTEGER DEFAULT 0,
            itens_lancados INTEGER DEFAULT 0,
            qtd_prevista REAL DEFAULT 0,
            qtd_lancada REAL DEFAULT 0,
            iniciado_em TEXT,
            finalizado_em TEXT,
            mensagem TEXT
        );
        CREATE TABLE IF NOT EXISTS agendamentos_automacao (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            lote TEXT NOT NULL,
            agendado_para TEXT NOT NULL,
            lojas_json TEXT NOT NULL,
            modos_json TEXT,
...
def _linhas_para_exportar_sistema_alvorada(separacoes: list[sqlite3.Row], itens_por_separacao: dict[int, list[sqlite3.Row]]) -> list[dict[str, Any]]:
    linhas: list[dict[str, Any]] = []
    for sep in separacoes:
        loja = _loja_codigo_para_sistema_alvorada(sep["store_nome"], sep["store_id"])
        for item in itens_por_separacao.get(sep["id"], []):
            quantidade = float(item["quantidade_separada"] or 0)
            if quantidade <= 0:
                quantidade = float(item["quantidade_pedida"] or 0)
            if quantidade <= 0:
                continue
            linhas.append(
                {
                    ...
                    "lote": sep["lote_nome"],
                    "loja": loja,
                    "produto": str(item["codigo"] or "").strip(),
                    "descricao": str(item["descricao"] or "").strip(),
                    "embalagem": float(item["fator_embalagem"] or 1),
                    "qtd": quantidade,
                }
            )
    return linhas
...
def gerar_db_sistema_alvorada_bytes(separacoes: list[sqlite3.Row], itens_por_separacao: dict[int, list[sqlite3.Row]]) -> bytes:
    linhas = _linhas_para_exportar_sistema_alvorada(separacoes, itens_por_separacao)
    if not linhas:
        raise ValueError("Não há itens com quantidade para exportar no formato sistema_alvorada.")

    lote_nome = separacoes[0]["lote_nome"] if separacoes else f"LOTE - {agora_br()}"
    lojas = sorted({linha["loja"] for linha in linhas}, key=lambda x: (len(str(x)), str(x)))
    ...
    try:
        ...
        try:
            criar_schema_sistema_alvorada(conn)
            agora = agora_br()
            for linha in linhas:
                conn.execute(
                    """
                    INSERT OR REPLACE INTO produtos
                    (item, descricao, embalagem, estoque_inicial, categoria, origem, codigo_barras, nivel_estoque, sugestao, estoque_ideal, atualizado_em)
                    VALUES (?, ?, ?, NULL, 'PRODUTO', 'SEPARACAO_WEB', NULL, NULL, NULL, NULL, ?)
                    """,
                    (linha["produto"], linha["descricao"], linha["embalagem"], agora),
                )
                conn.execute(
                    """
                    INSERT INTO pedidos_suspensos
                    (loja, produto, nome_produto, qtd, status, data_lancamento, qtd_lancada, ultima_execucao, mensagem_execucao)
                    VALUES (?, ?, ?, ?, 'PENDENTE', ?, 0, ?, ?)
                    """,
                    (
                        linha["loja"],
                        ...
                        linha["lote"],
                        agora,
                        "Gerado pelo sistema de separação após finalização.",
                    ),
                )
            cur = conn.execute(
                """
                INSERT INTO execucoes_lote
                (lote, iniciado_em, finalizado_em, status, lojas_previstas, lojas_concluidas, lojas_com_erro, mensagem)
                VALUES (?, ?, NULL, 'AGUARDANDO', ?, 0, 0, ?)
                """,
                (lote_nome, agora, len(lojas), "Arquivo gerado pelo sistema de separação; aguardando automação."),
            )
            execucao_id = cur.lastrowid
            for loja in lojas:
                linhas_loja = [linha for linha in linhas if linha["loja"] == loja]
                conn.execute(
                    """
                    INSERT INTO execucoes_lote_lojas
                    (execucao_id, lote, loja, codigo_site, nota_codigo, item_pausa_codigo, item_pausa_nome, item_pausa_indice,
                     status, itens_previstos, itens_lancados, qtd_prevista, qtd_lancada, iniciado_em, finalizado_em, mensagem)
                    VALUES (?, ?, ?, NULL, NULL, NULL, NULL, 0, 'PENDENTE', ?, 0, ?, 0, NULL, NULL, 'Aguardando lançamento pela automação.')
                    """,
                    (execucao_id, lote_nome, loja, len(linhas_loja), sum(float(l["qtd"] or 0) for l in linhas_loja)),
                )
            conn.commit()
        finally:
            conn.close()
...
def carregar_exportacao_sistema_alvorada_por_separacoes(separation_ids: list[int]) -> tuple[list[sqlite3.Row], dict[int, list[sqlite3.Row]]]:
    ids = [int(x) for x in separation_ids if str(x).isdigit() or isinstance(x, int)]
    if not ids:
        raise ValueError("Nenhuma separação selecionada para exportar.")
    placeholders = ",".join("?" for _ in ids)
    ...
    if not separacoes:
        raise ValueError("Separação não encontrada para exportar.")
    ...
    return separacoes, itens_por_separacao


def enviar_db_sistema_alvorada(separacoes: list[sqlite3.Row], itens_por_separacao: dict[int, list[sqlite3.Row]]) -> Response:
    data = gerar_db_sistema_alvorada_bytes(separacoes, itens_por_separacao)
    lote_nome = re.sub(r"[^A-Za-z0-9_-]+", "_", str(separacoes[0]["lote_nome"] if separacoes else "lote")).strip("_") or "lote"
    filename = f"sistema_alvorada_{lote_nome}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
    return send_file(io.BytesIO(data), as_attachment=True, download_name=filename, mimetype="application/x-sqlite3")
...
def finalizar_separacao(separation_id: int) -> Response:
    ...
    if request.form.get("gerar_sistema_alvorada") == "1":
        try:
            separacoes_export, itens_export = carregar_exportacao_sistema_alvorada_por_separacoes([separation_id])
            registrar_auditoria("exportar_sistema_alvorada_finalizacao", "separacao", str(separation_id), {"origem": "finalizacao"})
            return enviar_db_sistema_alvorada(separacoes_export, itens_export)
        except ValueError as exc:
            flash(f"Separação finalizada, mas não foi possível gerar o arquivo sistema_alvorada: {exc}", "error")
...
@app.get("/separacoes/<int:separation_id>/exportar-sistema-alvorada.db")
@login_required
@module_required("separacoes")
def exportar_sistema_alvorada_separacao(separation_id: int) -> Response:
    separation = load_separation(separation_id)
    if separation is None or not can_access_separation(separation):
        flash("Sem acesso a essa separação.", "error")
        return redirect(url_for("listar_separacoes"))
    if separation["status"] != "FINALIZADA":
        flash("Finalize a separação antes de exportar no formato sistema_alvorada.", "error")
        return redirect(url_for("detalhe_separacao", separation_id=separation_id))
    try:
        separacoes_export, itens_export = carregar_exportacao_sistema_alvorada_por_separacoes([separation_id])
        registrar_auditoria("exportar_sistema_alvorada_separacao", "separacao", str(separation_id), {})
        return enviar_db_sistema_alvorada(separacoes_export, itens_export)
    except ValueError as exc:
        flash(str(exc), "error")
        return redirect(url_for("detalhe_separacao", separation_id=separation_id))
...
@app.get("/relatorios/lotes/<lote_codigo>/exportar-sistema-alvorada.db")
@login_required
def exportar_sistema_alvorada_lote(lote_codigo: str) -> Response:
    if not user_has_access(g.user, "relatorios"):
        return forbidden_redirect("Sem permissão para exportar o histórico.")
    historico = [s for s in carregar_lote_completo(lote_codigo) if s["status"] == "FINALIZADA"]
    if not historico:
        flash("Nenhuma loja finalizada encontrada nesse lote para exportar.", "error")
        return redirect(url_for("relatorios"))
    try:
        separacoes_export, itens_export = carregar_exportacao_sistema_alvorada_por_separacoes([s["id"] for s in historico])
        registrar_auditoria("exportar_sistema_alvorada_lote", "lote", lote_codigo, {"lojas": len(historico)})
        return enviar_db_sistema_alvorada(separacoes_export, itens_export)
    except ValueError as exc:
        flash(str(exc), "error")
        return redirect(url_for("detalhe_historico_lote", lote_codigo=lote_codigo))
...
def _dados_relatorio_gerencial(periodo_dias: int = 30, linha_base: str = "Padaria", sublinha: str = "") -> dict[str, Any]:
    periodo = max(1, min(int(periodo_dias or 30), 365))
    linha_base = str(linha_base or "Padaria").strip() or "Padaria"
...
def _dados_resumo_padaria_industria_cd() -> dict[str, Any]:
    """Resumo simples da Padaria completa para a aba Relatórios.
...
def _dados_resumo_todas_linhas_estoque(filtro_linha: str = "") -> dict[str, Any]:
    """Calcula as linhas existentes no estoque atual, com filtro opcional por linha.

    A importação ERP pode preencher tanto linha_caminho_erp quanto linha_erp.
    Para o relatório geral usamos o caminho completo quando existir, e caímos
    para linha_erp/"Sem linha" quando o item não tiver caminho.
    """
    expr_linha = "COALESCE(NULLIF(TRIM(linha_caminho_erp), ''), NULLIF(TRIM(linha_erp), ''), 'Sem linha')"
    filtro_linha = (filtro_linha or "").strip()
    where_linha = ""
    params_linha: list[Any] = []
    if filtro_linha:
        where_linha = f" AND {expr_linha} = ?"
        params_linha.append(filtro_linha)
    with closing(get_conn()) as conn:
        opcoes_linhas = conn.execute(f"""
            SELECT {expr_linha} AS linha
            FROM stock_items
            WHERE ativo = 1
            GROUP BY {expr_linha}
            ORDER BY linha COLLATE NOCASE ASC
        ...
        resumo = conn.execute(f"""
            SELECT COUNT(*) AS produtos,
                   ...
                   COALESCE(SUM(quantidade_atual), 0) AS quantidade_total,
                   COALESCE(SUM(quantidade_atual * custo_unitario), 0) AS valor_estoque,
                   COALESCE(AVG(NULLIF(custo_unitario, 0)), 0) AS custo_medio,
                   COUNT(DISTINCT {expr_linha}) AS total_linhas
            FROM stock_items
            WHERE ativo = 1 {where_linha}
        ...
        linhas = conn.execute(f"""
            ...
            WHERE ativo = 1 {where_linha}
            GROUP BY {expr_linha}
            ORDER BY valor DESC, linha COLLATE NOCASE ASC
        """, params_linha).fetchall()
    return {"resumo": resumo, "linhas": linhas, "opcoes_linhas": opcoes_linhas, "filtro_linha": filtro_linha, "gerado_em": agora_br()}
...
def relatorios() -> str:
    if not user_has_access(g.user, "relatorios"):
        return forbidden_redirect("Sem permissão para acessar os relatórios.")
    filtro_linha = request.args.get("linha", "").strip()
    dados_padaria = _dados_resumo_padaria_industria_cd()
    dados_estoque_linhas = _dados_resumo_todas_linhas_estoque(filtro_linha)
    registrar_auditoria("visualizar_relatorios_estoque_todas_linhas", "relatorio", "estoque_todas_linhas", {"linha": filtro_linha})
    return render_template("relatorios.html", title="Relatórios", dados_padaria=dados_padaria, dados_estoque_linhas=dados_estoque_linhas)


@app.get("/relatorios/estoque-linhas/exportar.xlsx")
@login_required
@module_required("relatorios")
def exportar_relatorio_estoque_linhas_excel() -> Response:
    if not user_has_access(g.user, "relatorios"):
    ...
    filtro_linha = request.args.get("linha", "").strip()
    dados = _dados_resumo_todas_linhas_estoque(filtro_linha)
    wb = Workbook()
    ws = wb.active
    ws.title = "Linhas do estoque"
    ws.append(["Relatório de todas as linhas existentes no estoque"])
    ws.append(["Gerado em", dados["gerado_em"]])
    if dados.get("filtro_linha"):
        ws.append(["Filtro por linha", dados["filtro_linha"]])
    ws.append([])
    resumo = dados["resumo"]
    ws.append(["Total de linhas", resumo["total_linhas"] or 0])
    ws.append(["Itens existentes", resumo["produtos"] or 0])
    ws.append(["Quantidade total", float(resumo["quantidade_total"] or 0)])
    ws.append(["Valor estimado", float(resumo["valor_estoque"] or 0)])
    ws.append(["Abaixo do estoque", resumo["baixo"] or 0])
    ws.append(["Zerados", resumo["zerados"] or 0])
    ws.append([])
    headers = ["Linha", "Itens existentes", "Qtd. estoque", "Abaixo do estoque", "Zerados", "Custo médio", "Valor estimado", "Atualizado em"]
    ws.append(headers)
    header_row = ws.max_row
    for cell in ws[header_row]:
        cell.font = Font(bold=True, color="FFFFFF")
    ...
    for linha in dados["linhas"]:
        ws.append([
            linha["linha"],
            int(linha["produtos"] or 0),
            float(linha["saldo"] or 0),
            int(linha["baixo"] or 0),
            int(linha["zerados"] or 0),
            float(linha["custo_medio"] or 0),
            float(linha["valor"] or 0),
            linha["atualizado_em"] or "",
        ])
    for row in ws.iter_rows(min_row=header_row + 1, min_col=3, max_col=7):
        for cell in row:
    ...
    wb.save(buffer)
    buffer.seek(0)
    registrar_auditoria("exportar_relatorio_estoque_todas_linhas_excel", "relatorio", "estoque_todas_linhas", {"linha": filtro_linha})
    return send_file(buffer, mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", as_attachment=True, download_name=f"relatorio_estoque_todas_linhas_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx")
...
def _balance_item_nao_cadastrado(row: sqlite3.Row | dict[str, Any]) -> bool:
    try:
        return str(row["linha_erp"] or "").strip().upper() == "NÃO CADASTRADO"
    except Exception:
        return False
...
def detalhe_balanco(balance_id: int) -> str | Response:
    ...
    if request.method == "POST":
        ...
        with closing(get_conn()) as conn:
            ...
            agora = agora_iso()
            if item is None:
                descricao_temp = "Item não cadastrado - clique em Registrar para informar a descrição"
                cur = conn.execute(
                    """
                    INSERT INTO stock_items
                    (codigo, codigo_barras, descricao, fator_embalagem, quantidade_atual, custo_unitario, linha_erp, ativo, atualizado_em)
                    VALUES (?, '', ?, 1, 0, 0, 'NÃO CADASTRADO', 1, ?)
                    """,
                    (codigo, descricao_temp, agora),
                )
                stock_item_id = int(cur.lastrowid)
                conn.execute(
                    """
                    ...
                    (balance_count_id, stock_item_id, codigo, descricao, linha_erp, quantidade_sistema, quantidade_contada, delta, custo_unitario, criado_em, atualizado_em)
                    VALUES (?, ?, ?, ?, 'NÃO CADASTRADO', 0, ?, ?, 0, ?, ?)
                    """,
                    (balance_id, stock_item_id, codigo, descricao_temp, quantidade, quantidade, agora, agora),
                )
                flash("Item não existe no estoque, mas foi adicionado ao histórico do balanço. Clique em Registrar para cadastrar a descrição.", "warning")
            else:
                existente = conn.execute(
...
@app.post("/balanco/<int:balance_id>/item/<int:item_id>/editar-contagem")
@login_required
@module_required("balanco")
def editar_contagem_item_balanco(balance_id: int, item_id: int) -> Response:
    balanco = _balance_row(balance_id)
    if balanco is None or balanco["status"] != "ABERTO":
        flash("Não é possível editar item deste balanço.", "error")
        return redirect(url_for("balancos"))
    try:
        nova_contada = parse_quantidade_com_calculo(request.form.get("quantidade_contada", ""), "Quantidade contada")
    except ValueError as exc:
        flash(str(exc), "error")
        return redirect(url_for("detalhe_balanco", balance_id=balance_id))
    with closing(get_conn()) as conn:
        item = conn.execute("SELECT * FROM balance_count_items WHERE id = ? AND balance_count_id = ?", (item_id, balance_id)).fetchone()
        if item is None:
            flash("Item do balanço não encontrado.", "error")
            return redirect(url_for("detalhe_balanco", balance_id=balance_id))
        delta = nova_contada - float(item["quantidade_sistema"] or 0)
        conn.execute(
            "UPDATE balance_count_items SET quantidade_contada = ?, delta = ?, atualizado_em = ? WHERE id = ?",
            (nova_contada, delta, agora_iso(), item_id),
        )
        conn.commit()
    registrar_auditoria("editar_contagem_item_balanco", "balance_counts", str(balance_id), {"item_id": item_id, "quantidade": nova_contada})
    flash("Quantidade contada atualizada no histórico do balanço.", "success")
    return redirect(url_for("detalhe_balanco", balance_id=balance_id))


@app.post("/balanco/<int:balance_id>/item/<int:item_id>/registrar-produto")
@login_required
@module_required("balanco")
def registrar_produto_item_balanco(balance_id: int, item_id: int) -> Response:
    balanco = _balance_row(balance_id)
    if balanco is None or balanco["status"] != "ABERTO":
        flash("Não é possível registrar produto deste balanço.", "error")
        return redirect(url_for("balancos"))
    if not can_edit_stock_registration(g.user):
        return forbidden_redirect("Somente usuários com permissão de cadastro no estoque podem registrar este produto.")
    descricao = request.form.get("descricao", "").strip()
    codigo_barras = request.form.get("codigo_barras", "").strip()
    if not descricao:
        flash("Informe a descrição do item para registrar no estoque.", "error")
        return redirect(url_for("detalhe_balanco", balance_id=balance_id))
    with closing(get_conn()) as conn:
        bitem = conn.execute("SELECT * FROM balance_count_items WHERE id = ? AND balance_count_id = ?", (item_id, balance_id)).fetchone()
        if bitem is None:
            flash("Item do balanço não encontrado.", "error")
            return redirect(url_for("detalhe_balanco", balance_id=balance_id))
        codigo = str(bitem["codigo"] or "").strip()
        existente = conn.execute(
            "SELECT * FROM stock_items WHERE codigo = ? OR (codigo_barras IS NOT NULL AND codigo_barras <> '' AND codigo_barras = ?)",
            (codigo, codigo),
        ).fetchone()
        ...
        if existente:
            stock_item_id = int(existente["id"])
            sistema = float(existente["quantidade_atual"] or 0)
            custo = float(existente["custo_unitario"] or 0)
            linha = "" if str(existente["linha_erp"] or "").strip().upper() == "NÃO CADASTRADO" else existente["linha_erp"]
            conn.execute("UPDATE stock_items SET descricao = ?, codigo_barras = COALESCE(NULLIF(?, ''), codigo_barras), linha_erp = NULLIF(linha_erp, 'NÃO CADASTRADO'), ativo = 1, atualizado_em = ? WHERE id = ?", (descricao, codigo_barras, agora, stock_item_id))
        else:
            cur = conn.execute(
                "INSERT INTO stock_items (codigo, codigo_barras, descricao, fator_embalagem, quantidade_atual, custo_unitario, ativo, atualizado_em) VALUES (?, ?, ?, 1, 0, 0, 1, ?)",
                (codigo, codigo_barras, descricao, agora),
            )
            stock_item_id = int(cur.lastrowid)
            sistema = 0.0
            custo = 0.0
            linha = ""
        contada = float(bitem["quantidade_contada"] or 0)
        conn.execute(
            """
            UPDATE balance_count_items
            SET stock_item_id = ?, descricao = ?, linha_erp = ?, quantidade_sistema = ?, delta = ?, custo_unitario = ?, atualizado_em = ?
            WHERE id = ? AND balance_count_id = ?
            """,
            (stock_item_id, descricao, linha, sistema, contada - sistema, custo, agora, item_id, balance_id),
        )
        conn.commit()
    registrar_auditoria("registrar_produto_pelo_balanco", "balance_counts", str(balance_id), {"item_id": item_id, "codigo": codigo, "descricao": descricao})
    flash("Produto registrado no estoque. Agora você pode editar embalagem, custo e quantidade pela aba Estoque.", "success")
    return redirect(url_for("detalhe_balanco", balance_id=balance_id))
...
def confirmar_balanco(balance_id: int) -> Response:
    ...
    if not items:
        ...
        return redirect(url_for("detalhe_balanco", balance_id=balance_id))
    pendentes = [item for item in items if _balance_item_nao_cadastrado(item)]
    if pendentes:
        flash(f"Existem {len(pendentes)} item(ns) sem cadastro no estoque. Registre a descrição pelo histórico do balanço antes de confirmar.", "error")
        return redirect(url_for("detalhe_balanco", balance_id=balance_id))
    agora = agora_iso()
    with closing(get_conn()) as conn:
        for item in items:
...
@app.post("/comunicacao/grupos/<int:group_id>/limpar")
@login_required
@module_required("comunicacao")
def limpar_conversa_chat(group_id: int) -> Response:
    """Apaga todas as mensagens do chat selecionado sem excluir o grupo/conversa."""
    with closing(get_conn()) as conn:
        grupo = conn.execute("SELECT * FROM chat_groups WHERE id = ? AND ativo = 1", (group_id,)).fetchone()
        if grupo is None:
        ...
        arquivos = conn.execute(
            "SELECT arquivo_path FROM chat_messages WHERE group_id = ? AND arquivo_path IS NOT NULL AND arquivo_status = 'pendente'",
            (group_id,),
        ).fetchall()
        total = conn.execute("SELECT COUNT(*) AS total FROM chat_messages WHERE group_id = ?", (group_id,)).fetchone()["total"]

        for arq in arquivos:
            caminho = arq["arquivo_path"]
    ...
    registrar_auditoria(
        ...
        "chat_messages",
        str(group_id),
        {"grupo": grupo["nome"], "mensagens_apagadas": total, "arquivos_temporarios_apagados": len(arquivos)},
    )
    flash("Conversa limpa com sucesso. O chat continua disponível para novas mensagens.", "success")
    return redirect(url_for("comunicacao", group_id=group_id))
...
def enviar_mensagem_chat(group_id: int) -> Response:
    ...
    with closing(get_conn()) as conn:
        ...
        mid = cur.lastrowid
        if mensagem.lower().startswith("/agente"):
            try:
                _, resposta_agente = executar_comando_agente_no_texto(
                    mensagem,
                    system="Você é a IA offline/local do Sistema Alvorada dentro do chat. Responda em português do Brasil, com instruções práticas, organizadas e diretamente úteis para o trabalho do usuário.",
                )
                conn.execute(
                    """
                    INSERT INTO chat_messages (group_id, user_id, mensagem, arquivo_nome, arquivo_path, arquivo_status, expira_em, criado_em)
                    VALUES (?, NULL, ?, NULL, NULL, 'sem_arquivo', datetime('now', '+7 days'), ?)
                    """,
                    (group_id, "🤖 IA Alvorada:\n" + resposta_agente, agora_iso()),
                )
            except Exception as exc:
                conn.execute(
                    """
                    ...
                    VALUES (?, NULL, ?, NULL, NULL, 'sem_arquivo', datetime('now', '+7 days'), ?)
                    """,
                    (group_id, "🤖 IA Alvorada: " + str(exc), agora_iso()),
                )
        conn.commit()
    registrar_auditoria("enviar_mensagem_chat", "chat_messages", str(mid), {"group_id": group_id, "tem_arquivo": bool(arquivo_nome), "agente": mensagem.lower().startswith("/agente")})
    return redirect(url_for("comunicacao", group_id=group_id))
...
def buscar_item_estoque_por_codigo(conn: sqlite3.Connection, codigo: str) -> sqlite3.Row | None:
    ...
    if not codigo:
        return None
    return conn.execute(
        "SELECT * FROM stock_items WHERE ativo = 1 AND (codigo = ? OR codigo_barras = ?) LIMIT 1",
        (codigo, codigo),
    ).fetchone()


def calcular_ingredientes_ordem(conn: sqlite3.Connection, order_id: int) -> list[dict[str, Any]]:
    rows = conn.execute(
        """
        SELECT ri.ingredient_stock_item_id, si.codigo, si.descricao, ri.unidade,
               SUM(ri.quantidade * oi.quantidade_pedida) AS quantidade_total
        FROM production_order_items oi
        JOIN production_recipe_items ri ON ri.recipe_id = oi.recipe_id
        JOIN stock_items si ON si.id = ri.ingredient_stock_item_id
        WHERE oi.order_id = ?
        GROUP BY ri.ingredient_stock_item_id, si.codigo, si.descricao, ri.unidade
...
def receitas() -> str | Response:
    with closing(get_conn()) as conn:
        if request.method == 'POST':
            acao = request.form.get('acao', 'criar_receita')
            if acao == 'criar_receita':
                codigo_produto = request.form.get('codigo_produto', '').strip()
                observacao = request.form.get('observacao', '').strip()
                produto = buscar_item_estoque_por_codigo(conn, codigo_produto)
                if produto is None:
                ...
                try:
                    cur = conn.execute(
                        """
                        INSERT INTO production_recipes (product_stock_item_id, nome, observacao, ativo, criado_por, criado_em, atualizado_em)
                        VALUES (?, ?, ?, 1, ?, ?, ?)
                        ON CONFLICT(product_stock_item_id) DO UPDATE SET
                            nome = excluded.nome,
                            observacao = excluded.observacao,
                            ativo = 1,
                            atualizado_em = excluded.atualizado_em
                        """,
                        (produto['id'], produto['descricao'], observacao, g.user['id'], agora_iso(), agora_iso()),
                    )
                    conn.commit()
                    registrar_auditoria('salvar_receita', 'production_recipes', str(cur.lastrowid or produto['id']), {'codigo_produto': codigo_produto})
                    flash('Receita salva. Agora adicione os ingredientes abaixo.', 'success')
                except sqlite3.IntegrityError:
                    flash('Não foi possível salvar a receita.', 'error')
                return redirect(url_for('receitas'))

            if acao == 'criar_ordem':
                recipe_id_raw = request.form.get('recipe_id', '').strip()
                quantidade_raw = request.form.get('quantidade_pedida', '').strip()
                store_id_raw = request.form.get('store_id', '').strip()
                titulo = request.form.get('titulo', '').strip() or f'Ordem de produção {agora_br()}'
                observacao = request.form.get('observacao_ordem', '').strip()
                if not recipe_id_raw.isdigit():
                    flash('Escolha uma receita para montar a ordem de produção.', 'error')
                    return redirect(url_for('receitas'))
                try:
                    quantidade = parse_quantidade_com_calculo(quantidade_raw, 'Quantidade do pedido')
                except ValueError as exc:
                    flash(str(exc), 'error')
                    return redirect(url_for('receitas'))
                receita = conn.execute(
                    """
                    SELECT r.*, si.codigo, si.descricao
                    FROM production_recipes r
                    JOIN stock_items si ON si.id = r.product_stock_item_id
                ...
                cur = conn.execute(
                    """
                    INSERT INTO production_orders (titulo, store_id, status, observacao, criado_por, criado_em)
                    VALUES (?, ?, 'ABERTA', ?, ?, ?)
                    """,
                    (titulo, store_id, observacao, g.user['id'], agora_iso()),
                )
                order_id = int(cur.lastrowid)
                conn.execute(
                    """
                    INSERT INTO production_order_items (order_id, recipe_id, product_stock_item_id, codigo, descricao, quantidade_pedida, quantidade_produzida, status, atualizado_em)
                    VALUES (?, ?, ?, ?, ?, ?, 0, 'PENDENTE', ?)
                    """,
                    (order_id, receita['id'], receita['product_stock_item_id'], receita['codigo'], receita['descricao'], quantidade, agora_iso()),
                )
                conn.execute(
                    """
                    INSERT INTO production_order_logs (order_id, user_id, action, observacao, criado_em)
                    VALUES (?, ?, 'CRIAR_ORDEM', ?, ?)
                    """,
                    (order_id, g.user['id'], f'{fmt_num(quantidade)} x {receita["descricao"]}', agora_iso()),
                )
                conn.commit()
                registrar_auditoria('criar_ordem_producao', 'production_orders', str(order_id), {'recipe_id': int(recipe_id_raw), 'quantidade': quantidade})
                flash('Ordem de produção criada.', 'success')
                return redirect(url_for('ordem_producao', order_id=order_id))

        receitas_rows = conn.execute(
            """
            SELECT r.*, si.codigo, si.descricao, si.custo_unitario,
                   COUNT(ri.id) AS total_ingredientes
            FROM production_recipes r
            JOIN stock_items si ON si.id = r.product_stock_item_id
            LEFT JOIN production_recipe_items ri ON ri.recipe_id = r.id
            WHERE r.ativo = 1
            GROUP BY r.id
            ORDER BY si.descricao
            """
        ).fetchall()
        receita_ids = [int(r['id']) for r in receitas_rows]
        ingredientes_por_receita: dict[int, list[sqlite3.Row]] = {}
        if receita_ids:
            placeholders = ','.join('?' for _ in receita_ids)
            ing_rows = conn.execute(
                f"""
                SELECT ri.*, si.codigo, si.descricao
            ...
            for row in ing_rows:
                ingredientes_por_receita.setdefault(int(row['recipe_id']), []).append(row)
        ordens = conn.execute(
            """
            SELECT o.*, st.nome AS store_nome, u.nome AS criado_por_nome,
                   COUNT(oi.id) AS total_itens,
                   SUM(oi.quantidade_pedida) AS total_pedido,
                   SUM(oi.quantidade_produzida) AS total_produzido
            FROM production_orders o
            LEFT JOIN stores st ON st.id = o.store_id
            LEFT JOIN users u ON u.id = o.criado_por
            LEFT JOIN production_order_items oi ON oi.order_id = o.id
            GROUP BY o.id
            ORDER BY o.id DESC
            LIMIT 40
            """
...
@login_required
@module_required('receitas')
def remover_receita(recipe_id: int) -> Response:
    """Remove uma receita criada sem apagar o histórico de produção já registrado.

    A remoção é feita como inativação (ativo = 0), para preservar ordens antigas,
    relatórios mensais e auditoria. Os ingredientes vinculados são removidos da
    receita para ela não aparecer mais na tela nem nos novos pedidos.
    """
    with closing(get_conn()) as conn:
        receita = conn.execute(
            """
            SELECT r.id, r.product_stock_item_id, si.codigo, si.descricao
            FROM production_recipes r
            JOIN stock_items si ON si.id = r.product_stock_item_id
            WHERE r.id = ? AND r.ativo = 1
            """,
        ...
        if receita is None:
            flash('Receita não encontrada ou já removida.', 'error')
            return redirect(url_for('receitas'))

        conn.execute('DELETE FROM production_recipe_items WHERE recipe_id = ?', (recipe_id,))
        conn.execute('UPDATE production_recipes SET ativo = 0, atualizado_em = ? WHERE id = ?', (agora_iso(), recipe_id))
        conn.commit()
    ...
    registrar_auditoria(
        ...
        'production_recipes',
        str(recipe_id),
        {'codigo_produto': receita['codigo'], 'descricao': receita['descricao']},
    )
    flash('Receita removida. O histórico de produção anterior foi preservado.', 'success')
    return redirect(url_for('receitas'))


@app.post('/receitas/<int:recipe_id>/ingredientes')
@login_required
@module_required('receitas')
def adicionar_ingrediente_receita(recipe_id: int) -> Response:
    codigo = request.form.get('codigo_ingrediente', '').strip()
    unidade = request.form.get('unidade', 'kg').strip().lower() or 'kg'
    if unidade not in {'kg', 'g', 'l', 'ml', 'un'}:
        unidade = 'kg'
    try:
        quantidade = parse_quantidade_com_calculo(request.form.get('quantidade', ''), 'Quantidade do ingrediente')
    except ValueError as exc:
        flash(str(exc), 'error')
        return redirect(url_for('receitas'))
    with closing(get_conn()) as conn:
        receita = conn.execute('SELECT * FROM production_recipes WHERE id = ? AND ativo = 1', (recipe_id,)).fetchone()
        ingrediente = buscar_item_estoque_por_codigo(conn, codigo)
        if receita is None or ingrediente is None:
            flash('Receita ou ingrediente não encontrado.', 'error')
            return redirect(url_for('receitas'))
        conn.execute(
            """
            INSERT INTO production_recipe_items (recipe_id, ingredient_stock_item_id, quantidade, unidade, criado_em)
            VALUES (?, ?, ?, ?, ?)
            ON CONFLICT(recipe_id, ingredient_stock_item_id) DO UPDATE SET
                quantidade = excluded.quantidade,
                unidade = excluded.unidade
            """,
            (recipe_id, ingrediente['id'], quantidade, unidade, agora_iso()),
        )
        conn.execute('UPDATE production_recipes SET atualizado_em = ? WHERE id = ?', (agora_iso(), recipe_id))
        conn.commit()
    registrar_auditoria('adicionar_ingrediente_receita', 'production_recipes', str(recipe_id), {'codigo': codigo, 'quantidade': quantidade, 'unidade': unidade})
    flash('Ingrediente vinculado à receita.', 'success')
    return redirect(url_for('receitas'))


@app.post('/receitas/ingredientes/<int:item_id>/remover')
@login_required
@module_required('receitas')
def remover_ingrediente_receita(item_id: int) -> Response:
    with closing(get_conn()) as conn:
        row = conn.execute('SELECT recipe_id FROM production_recipe_items WHERE id = ?', (item_id,)).fetchone()
        if row:
            conn.execute('DELETE FROM production_recipe_items WHERE id = ?', (item_id,))
            conn.execute('UPDATE production_recipes SET atualizado_em = ? WHERE id = ?', (agora_iso(), row['recipe_id']))
            conn.commit()
    flash('Ingrediente removido.', 'success')
    return redirect(url_for('receitas'))


@app.route('/ordem-producao/<int:order_id>', methods=['GET', 'POST'])
@login_required
@module_required('receitas')
def ordem_producao(order_id: int) -> str | Response:
    with closing(get_conn()) as conn:
        ordem = conn.execute(
            """
            SELECT o.*, st.nome AS store_nome, u.nome AS criado_por_nome
        ...
        if ordem is None:
            ...
            return redirect(url_for('receitas'))
        if request.method == 'POST':
            acao = request.form.get('acao', 'atualizar_item')
            if acao == 'fechar_ordem':
                conn.execute("UPDATE production_orders SET status = 'FINALIZADA', fechado_por = ?, fechado_em = ? WHERE id = ?", (g.user['id'], agora_iso(), order_id))
                conn.execute("INSERT INTO production_order_logs (order_id, user_id, action, observacao, criado_em) VALUES (?, ?, 'FINALIZAR_ORDEM', '', ?)", (order_id, g.user['id'], agora_iso()))
                conn.commit()
                registrar_auditoria('finalizar_ordem_producao', 'production_orders', str(order_id), {})
                flash('Ordem finalizada e registrada no histórico.', 'success')
                return redirect(url_for('ordem_producao', order_id=order_id))
            item_id = request.form.get('item_id', '').strip()
            if not item_id.isdigit():
                flash('Item da ordem inválido.', 'error')
                return redirect(url_for('ordem_producao', order_id=order_id))
            item = conn.execute('SELECT * FROM production_order_items WHERE id = ? AND order_id = ?', (int(item_id), order_id)).fetchone()
            if item is None:
                flash('Item da ordem não encontrado.', 'error')
                return redirect(url_for('ordem_producao', order_id=order_id))
            try:
                quantidade_produzida = parse_quantidade_com_calculo(request.form.get('quantidade_produzida', ''), 'Quantidade produzida')
            except ValueError as exc:
                flash(str(exc), 'error')
                return redirect(url_for('ordem_producao', order_id=order_id))
            status = 'CONCLUIDO' if quantidade_produzida >= float(item['quantidade_pedida'] or 0) else ('EM_PRODUCAO' if quantidade_produzida > 0 else 'PENDENTE')
            conn.execute('UPDATE production_order_items SET quantidade_produzida = ?, status = ?, atualizado_em = ? WHERE id = ?', (quantidade_produzida, status, agora_iso(), int(item_id)))
            conn.execute("INSERT INTO production_order_logs (order_id, order_item_id, user_id, action, quantidade, observacao, criado_em) VALUES (?, ?, ?, 'ATUALIZAR_PRODUCAO', ?, ?, ?)", (order_id, int(item_id), g.user['id'], quantidade_produzida, request.form.get('observacao', '').strip(), agora_iso()))
            pendentes = conn.execute("SELECT COUNT(*) AS c FROM production_order_items WHERE order_id = ? AND status != 'CONCLUIDO'", (order_id,)).fetchone()['c']
            conn.execute("UPDATE production_orders SET status = ? WHERE id = ?", ('CONCLUIDA' if int(pendentes or 0) == 0 else 'EM_PRODUCAO', order_id))
            conn.commit()
            flash('Produção conferida e registrada.', 'success')
            return redirect(url_for('ordem_producao', order_id=order_id))
        itens = conn.execute('SELECT * FROM production_order_items WHERE order_id = ? ORDER BY descricao', (order_id,)).fetchall()
        ingredientes = calcular_ingredientes_ordem(conn, order_id)
        logs = conn.execute(
            """
            SELECT l.*, u.nome AS usuario_nome, oi.descricao AS item_descricao
            FROM production_order_logs l
            LEFT JOIN users u ON u.id = l.user_id
            LEFT JOIN production_order_items oi ON oi.id = l.order_item_id
            WHERE l.order_id = ?
            ORDER BY l.id DESC
            """,
            (order_id,),
        ).fetchall()
    return render_template('ordem_producao.html', title='Ordem de produção', ordem=ordem, itens=itens, ingredientes=ingredientes, logs=logs)


@app.post('/ordem-producao/<int:order_id>/remover')
@login_required
@module_required('receitas')
def remover_ordem_producao(order_id: int) -> Response:
    """Remove uma ordem de produção criada.

    A exclusão remove os itens e logs vinculados da ordem para ela deixar de
    aparecer na tela e no histórico CSV. Antes de excluir, os dados principais
    são enviados para auditoria para manter rastreabilidade administrativa.
    """
    with closing(get_conn()) as conn:
        ordem = conn.execute(
            """
        ...
        if ordem is None:
            flash('Ordem de produção não encontrada ou já removida.', 'error')
            return redirect(url_for('receitas'))

        itens = conn.execute(
            """
            SELECT codigo, descricao, quantidade_pedida, quantidade_produzida, status
        ...
        resumo_itens = [
            {
                ...
                'quantidade_pedida': float(i['quantidade_pedida'] or 0),
                'quantidade_produzida': float(i['quantidade_produzida'] or 0),
                'status': i['status'],
            }
            for i in itens
        ]

        # Remoção explícita para funcionar mesmo quando o banco SQLite não
        # estiver com foreign_keys=ON na conexão atual.
        conn.execute('DELETE FROM production_order_logs WHERE order_id = ?', (order_id,))
        conn.execute('DELETE FROM production_order_items WHERE order_id = ?', (order_id,))
        conn.execute('DELETE FROM production_orders WHERE id = ?', (order_id,))
        conn.commit()

    registrar_auditoria(
        'remover_ordem_producao',
        'production_orders',
        str(order_id),
        {
            'titulo': ordem['titulo'],
            'loja': ordem['store_nome'] or '',
            'status': ordem['status'],
            'criado_em': ordem['criado_em'],
            'criado_por': ordem['criado_por_nome'] or '',
            'itens': resumo_itens,
        },
    )
    flash('Ordem de produção removida. O registro da remoção foi salvo na auditoria.', 'success')
    return redirect(url_for('receitas'))


@app.get('/receitas/historico.csv')
@login_required
@module_required('receitas')
def exportar_historico_producao_csv() -> Response:
    data_inicial = request.args.get('data_inicial', '').strip()
    data_final = request.args.get('data_final', '').strip()
    params: list[Any] = []
    where = []
    if data_inicial:
        where.append('date(o.criado_em) >= date(?)')
    ...
    with closing(get_conn()) as conn:
        rows = conn.execute(
            f"""
            SELECT o.id, o.titulo, o.status, o.criado_em, o.fechado_em, st.nome AS loja,
                   oi.codigo, oi.descricao, oi.quantidade_pedida, oi.quantidade_produzida, oi.status AS status_item,
                   u.nome AS criado_por_nome
            FROM production_orders o
            LEFT JOIN stores st ON st.id = o.store_id
            LEFT JOIN users u ON u.id = o.criado_por
            JOIN production_order_items oi ON oi.order_id = o.id
            {sql_where}
            ORDER BY o.criado_em DESC, o.id DESC, oi.descricao
            """,
            params,
        ).fetchall()
    output = io.StringIO()
    writer = csv.writer(output, delimiter=';')
    writer.writerow(['ordem_id', 'titulo', 'loja', 'status_ordem', 'criado_em', 'fechado_em', 'codigo', 'produto', 'qtd_pedida', 'qtd_produzida', 'status_item', 'criado_por'])
    for r in rows:
        writer.writerow([r['id'], r['titulo'], r['loja'] or '', r['status'], r['criado_em'], r['fechado_em'] or '', r['codigo'], r['descricao'], fmt_num(r['quantidade_pedida']), fmt_num(r['quantidade_produzida']), r['status_item'], r['criado_por_nome'] or ''])
    return Response(output.getvalue(), mimetype='text/csv; charset=utf-8', headers={'Content-Disposition': 'attachment; filename=historico_producao.csv'})


# =========================
# Área do usuário: tarefas, metas, horários e notas pessoais
# =========================

def _normalizar_datetime_local(value: Any) -> str:
    texto = str(value or "").strip()
    if not texto:
        return ""
    texto = texto.replace(" ", "T")[:16]
    try:
...
def _fmt_dt_local(value: Any) -> str:
    texto = str(value or "").strip()
    if not texto:
        return ""
...
def _http_json_local(url: str, payload: dict[str, Any] | None = None, timeout: float = 2.5) -> dict[str, Any] | None:
    """Chama serviços locais de IA sem enviar dados para a internet."""
    data = None
    method = "GET"
...
def _scan_arquivos_modelos_locais(limit: int = 80) -> list[dict[str, str]]:
    """Procura arquivos comuns de modelos offline no PC onde o servidor Flask está rodando."""
    extensoes = {".gguf", ".ggml", ".bin", ".safetensors"}
    home = Path.home()
    candidatos = [
        home / "Downloads",
        home / "Documents",
        ...
        try:
            for arquivo in pasta.rglob("*"):
                ...
                try:
                    ...
                    tamanho_mb = 0
                encontrados.append({
                    "id": f"arquivo::{caminho}",
                    "nome": arquivo.name,
                    "origem": "Arquivo local",
                    "detalhe": f"{caminho} ({tamanho_mb:.0f} MB)",
                    "geravel": "nao",
                })
        except Exception:
            continue
...
def detectar_modelos_ia_locais() -> list[dict[str, str]]:
    """Detecta modelos conectáveis via Ollama/LM Studio e arquivos locais comuns."""
    modelos: list[dict[str, str]] = []
    vistos: set[str] = set()

    ollama = _http_json_local("http://127.0.0.1:11434/api/tags", timeout=1.5)
    if isinstance(ollama, dict):
        for m in ollama.get("models", []) or []:
            ...
            vistos.add(key)
            detalhes = []
            if m.get("details", {}).get("family"):
                detalhes.append(str(m["details"].get("family")))
            if m.get("size"):
                try:
                    detalhes.append(f"{float(m['size']) / (1024**3):.1f} GB")
                except Exception:
                    pass
            modelos.append({"id": key, "nome": nome, "origem": "Ollama", "detalhe": " • ".join(detalhes), "geravel": "sim"})

    lmstudio = _http_json_local("http://127.0.0.1:1234/v1/models", timeout=1.5)
    if isinstance(lmstudio, dict):
        for m in lmstudio.get("data", []) or []:
            nome = str(m.get("id") or "").strip()
            if not nome:
                continue
            key = f"lmstudio::{nome}"
            if key in vistos:
                continue
            vistos.add(key)
            modelos.append({"id": key, "nome": nome, "origem": "LM Studio", "detalhe": "Servidor local OpenAI compatível", "geravel": "sim"})

    for m in _scan_arquivos_modelos_locais():
...
Não invente dados externos. Estruture com:
- Objetivo
- Passos de execução
- Cuidados importantes
- Critério para considerar concluída
- Pendências para anotar, se houver

Tarefa: {contexto['titulo']}
Meta: {contexto['meta'] or 'não informada'}
...
""".strip()
    if modelo_id.startswith("ollama::"):
        ...
        texto = str((data or {}).get("response") or "").strip()
        if texto:
            return texto
        raise RuntimeError("O Ollama não retornou texto. Confira se o modelo está carregado corretamente.")
    if modelo_id.startswith("lmstudio::"):
        nome = modelo_id.split("::", 1)[1]
        data = _http_json_local(
            "http://127.0.0.1:1234/v1/chat/completions",
            {
                "model": nome,
                "messages": [
                    {"role": "system", "content": "Você cria observações operacionais curtas, claras e práticas em português do Brasil."},
        ...
        try:
            texto = str(data["choices"][0]["message"]["content"]).strip() if data else ""
        except Exception:
            texto = ""
        if texto:
            return texto
        raise RuntimeError("O LM Studio não retornou texto. Confira se o servidor local está iniciado.")
    raise RuntimeError("Esse item foi detectado como arquivo local, mas ainda precisa estar aberto em um servidor como Ollama ou LM Studio para gerar observações.")


def gerar_resposta_com_modelo_local(modelo_id: str, mensagem: str, system: str = "") -> str:
    """Gera resposta com o modelo offline selecionado nas Configurações."""
    mensagem = (mensagem or "").strip()
    if not mensagem:
        raise RuntimeError("Digite uma pergunta para a IA.")
    system = (system or "Você é a IA Alvorada integrada ao sistema. Responda em português do Brasil, de forma prática, segura e objetiva.").strip()
    prompt = f"{system}\n\nPergunta do usuário:\n{mensagem}"
    if modelo_id.startswith("ollama::"):
        nome = modelo_id.split("::", 1)[1]
        data = _http_json_local(
        ...
        if texto:
            return texto
        raise RuntimeError("O Ollama não retornou resposta. Confira se o Ollama está aberto e se o modelo existe.")
    if modelo_id.startswith("lmstudio::"):
        nome = modelo_id.split("::", 1)[1]
        data = _http_json_local(
            "http://127.0.0.1:1234/v1/chat/completions",
        ...
        if texto:
            return texto
        raise RuntimeError("O LM Studio não retornou resposta. Confira se o servidor local está iniciado.")
    raise RuntimeError("Modelo local selecionado não está conectável. Use um modelo do Ollama ou LM Studio em execução.")


def modelo_offline_configurado() -> str:
    if get_setting("offline_ai_enabled", "0") != "1":
        return ""
    return get_setting("offline_ai_model_id", "").strip()


def responder_ia_sistema(mensagem: str, system: str = "") -> str:
    modelo_id = modelo_offline_configurado()
    if modelo_id:
        return gerar_resposta_com_modelo_local(modelo_id, mensagem, system=system)
    provider = get_setting("cloud_ai_provider", "openai").strip().lower()
    if provider == "gemini":
        return chamar_gemini_simples(mensagem, system=system)
    if get_openai_api_key():
        return chamar_openai_simples(mensagem, system=system)
    if get_google_ai_api_key():
        return chamar_gemini_simples(mensagem, system=system)
    return chamar_openai_simples(mensagem, system=system)


def executar_comando_agente_no_texto(texto: str, system: str = "") -> tuple[bool, str]:
    """Se o texto começar com /agente, executa a IA em cima do que vem depois."""
    texto = (texto or "").strip()
    if not texto.lower().startswith("/agente"):
        return False, texto
    pergunta = texto[len("/agente"):].strip(" :\n\t")
    if not pergunta:
        raise RuntimeError("Digite a pergunta na frente de /agente. Exemplo: /agente crie uma observação para conferir estoque.")
    resposta = responder_ia_sistema(pergunta, system=system or "Você é um agente local do Sistema Alvorada. Responda em português, com passos práticos e aplicáveis ao trabalho do usuário.")
    return True, resposta


@app.route("/area-usuario", methods=["GET", "POST"])
@login_required
@module_required("area_usuario")
def area_usuario() -> str | Response:
    uid = int(g.user["id"])
    if request.method == "POST":
        titulo = request.form.get("titulo", "").strip()
        meta = request.form.get("meta", "").strip()
        horario_inicio = _normalizar_datetime_local(request.form.get("horario_inicio", ""))
        prazo = _normalizar_datetime_local(request.form.get("prazo", ""))
        ...
        with closing(get_conn()) as conn:
            cur = conn.execute(
                """
                INSERT INTO user_area_tasks (user_id, titulo, meta, horario_inicio, prazo, anotacoes, status, criado_em, atualizado_em)
                VALUES (?, ?, ?, ?, ?, ?, 'ABERTA', ?, ?)
                """,
                (uid, titulo, meta, horario_inicio, prazo, anotacoes, agora_iso(), agora_iso()),
            )
            conn.commit()
        registrar_auditoria("criar_tarefa_area_usuario", "user_area_tasks", str(cur.lastrowid), {"titulo": titulo, "prazo": prazo})
        flash("Tarefa criada na sua área.", "success")
        return redirect(url_for("area_usuario"))
    ...
    with closing(get_conn()) as conn:
        tarefas = conn.execute(
            """
            SELECT *
            FROM user_area_tasks
            WHERE user_id = ?
            ORDER BY CASE status WHEN 'ABERTA' THEN 0 WHEN 'ATRASADA' THEN 1 ELSE 2 END,
                     CASE WHEN prazo IS NULL OR TRIM(prazo) = '' THEN 1 ELSE 0 END,
                     prazo ASC,
                     id DESC
            """,
            (uid,),
        ...
        notas = conn.execute(
            "SELECT * FROM user_area_notes WHERE user_id = ? ORDER BY atualizado_em DESC, id DESC LIMIT 50",
            (uid,),
        ).fetchall()
    agora_txt = datetime.now().strftime("%Y-%m-%dT%H:%M")
    resumo = {"abertas": 0, "atrasadas": 0, "concluidas": 0}
    tarefas_view = []
    agora_dt = datetime.now()
    for tarefa in tarefas:
        status = tarefa["status"] or "ABERTA"
        atrasada = False
        if status == "ABERTA" and tarefa["prazo"]:
            try:
                atrasada = datetime.strptime(str(tarefa["prazo"])[:16], "%Y-%m-%dT%H:%M") < agora_dt
            except ValueError:
                atrasada = False
        if atrasada:
            resumo["atrasadas"] += 1
        ...
        tarefas_view.append({
            "row": tarefa,
            "atrasada": atrasada,
            "inicio_fmt": _fmt_dt_local(tarefa["horario_inicio"]),
            "prazo_fmt": _fmt_dt_local(tarefa["prazo"]),
        })
    return render_template("area_usuario.html", title="Área do usuário", tarefas=tarefas_view, notas=notas, resumo=resumo, agora_txt=agora_txt)


@app.post("/area-usuario/tarefas/<int:task_id>/editar")
@login_required
@module_required("area_usuario")
def editar_tarefa_area_usuario(task_id: int) -> Response:
    uid = int(g.user["id"])
    titulo = request.form.get("titulo", "").strip()
    meta = request.form.get("meta", "").strip()
    horario_inicio = _normalizar_datetime_local(request.form.get("horario_inicio", ""))
    ...
    if not titulo:
        ...
        return redirect(url_for("area_usuario"))
    with closing(get_conn()) as conn:
        row = conn.execute("SELECT id FROM user_area_tasks WHERE id = ? AND user_id = ?", (task_id, uid)).fetchone()
        if row is None:
            flash("Tarefa não encontrada.", "error")
            return redirect(url_for("area_usuario"))
        conn.execute(
            """
            UPDATE user_area_tasks
            SET titulo = ?, meta = ?, horario_inicio = ?, prazo = ?, anotacoes = ?, notificado_em = NULL, atualizado_em = ?
            WHERE id = ? AND user_id = ?
            """,
            (titulo, meta, horario_inicio, prazo, anotacoes, agora_iso(), task_id, uid),
        )
        conn.commit()
...
@app.post("/area-usuario/tarefas/<int:task_id>/concluir")
@login_required
@module_required("area_usuario")
def concluir_tarefa_area_usuario(task_id: int) -> Response:
    uid = int(g.user["id"])
    with closing(get_conn()) as conn:
        conn.execute(
            "UPDATE user_area_tasks SET status = 'CONCLUIDA', concluido_em = ?, atualizado_em = ? WHERE id = ? AND user_id = ?",
            (agora_iso(), agora_iso(), task_id, uid),
        )
        conn.commit()
    flash("Tarefa concluída.", "success")
    return redirect(url_for("area_usuario"))
...
@app.post("/area-usuario/tarefas/<int:task_id>/reabrir")
@login_required
@module_required("area_usuario")
def reabrir_tarefa_area_usuario(task_id: int) -> Response:
    uid = int(g.user["id"])
    with closing(get_conn()) as conn:
        conn.execute(
            "UPDATE user_area_tasks SET status = 'ABERTA', concluido_em = NULL, notificado_em = NULL, atualizado_em = ? WHERE id = ? AND user_id = ?",
            (agora_iso(), task_id, uid),
        )
        conn.commit()
...
@app.post("/area-usuario/tarefas/<int:task_id>/excluir")
@login_required
@module_required("area_usuario")
def excluir_tarefa_area_usuario(task_id: int) -> Response:
    uid = int(g.user["id"])
    with closing(get_conn()) as conn:
        conn.execute("DELETE FROM user_area_tasks WHERE id = ? AND user_id = ?", (task_id, uid))
        conn.commit()
    flash("Tarefa removida.", "success")
    return redirect(url_for("area_usuario"))


@app.post("/area-usuario/notas/criar")
@login_required
@module_required("area_usuario")
def criar_nota_area_usuario() -> Response:
    uid = int(g.user["id"])
    titulo = request.form.get("titulo", "").strip() or "Anotação"
    conteudo = request.form.get("conteudo", "").strip()
    if not conteudo:
        flash("Escreva a anotação.", "error")
        return redirect(url_for("area_usuario"))
    with closing(get_conn()) as conn:
        conn.execute(
            "INSERT INTO user_area_notes (user_id, titulo, conteudo, criado_em, atualizado_em) VALUES (?, ?, ?, ?, ?)",
            (uid, titulo, conteudo, agora_iso(), agora_iso()),
        )
        conn.commit()
...
@app.post("/area-usuario/notas/<int:note_id>/editar")
@login_required
@module_required("area_usuario")
def editar_nota_area_usuario(note_id: int) -> Response:
    uid = int(g.user["id"])
    titulo = request.form.get("titulo", "").strip() or "Anotação"
    conteudo = request.form.get("conteudo", "").strip()
    if not conteudo:
    ...
    with closing(get_conn()) as conn:
        conn.execute(
            "UPDATE user_area_notes SET titulo = ?, conteudo = ?, atualizado_em = ? WHERE id = ? AND user_id = ?",
            (titulo, conteudo, agora_iso(), note_id, uid),
        )
        conn.commit()
...
@app.post("/area-usuario/notas/<int:note_id>/excluir")
@login_required
@module_required("area_usuario")
def excluir_nota_area_usuario(note_id: int) -> Response:
    uid = int(g.user["id"])
    with closing(get_conn()) as conn:
        conn.execute("DELETE FROM user_area_notes WHERE id = ? AND user_id = ?", (note_id, uid))
        conn.commit()
...
@app.get("/api/area-usuario/modelos-locais")
@login_required
@module_required("area_usuario")
def api_modelos_locais_area_usuario() -> Response:
    modelos = detectar_modelos_ia_locais()
    return jsonify({"ok": True, "modelos": modelos})


@app.post("/api/area-usuario/gerar-observacao")
@login_required
@module_required("area_usuario")
...
@app.get("/api/area-usuario/tarefas-vencidas")
@login_required
@module_required("area_usuario")
def api_tarefas_vencidas_area_usuario() -> Response:
    uid = int(g.user["id"])
    agora_local = datetime.now().strftime("%Y-%m-%dT%H:%M")
    with closing(get_conn()) as conn:
        rows = conn.execute(
            """
            SELECT id, titulo, meta, prazo
            FROM user_area_tasks
            WHERE user_id = ?
              AND status = 'ABERTA'
    ...
    return jsonify({
        "ok": True,
        "tarefas": [
            {"id": r["id"], "titulo": r["titulo"], "meta": r["meta"] or "", "prazo": _fmt_dt_local(r["prazo"])}
            for r in rows
        ],
    })


@app.post("/api/area-usuario/tarefas/<int:task_id>/notificada")
@login_required
@module_required("area_usuario")
def api_marcar_tarefa_notificada_area_usuario(task_id: int) -> Response:
    uid = int(g.user["id"])
    with closing(get_conn()) as conn:
        conn.execute(
            "UPDATE user_area_tasks SET notificado_em = ?, atualizado_em = ? WHERE id = ? AND user_id = ? AND status = 'ABERTA'",
            (agora_iso(), agora_iso(), task_id, uid),
        )
        conn.commit()
